...
export interface ImagePatch {
  ...
  flipH?: boolean
  flipV?: boolean
  /** non-destructive source crop; null removes a:srcRect, undefined keeps */
...
export function patchImageParagraphXml(xml: string, patch: ImagePatch): string {
  ...
  if (patch.rotDeg !== undefined || patch.flipH !== undefined || patch.flipV !== undefined) {
    // Some valid producers omit pic:spPr/a:xfrm when the picture has no local
    // transform. Materialize the minimal picture transform before applying a
    // requested final rotation/flip so the edit cannot disappear on save.
    if (!/<pic:spPr[^>]*>[\s\S]*?<a:xfrm\b/.test(out)) {
      const extent = /<wp:extent[^>]*?\bcx="(\d+)"[^>]*?\bcy="(\d+)"/.exec(out)
      const cx = extent?.[1] ?? '1'
      const cy = extent?.[2] ?? '1'
      const xfrm = `<a:xfrm><a:off x="0" y="0"/><a:ext cx="${cx}" cy="${cy}"/></a:xfrm>`
      if (/<pic:spPr[^>]*\/>/.test(out)) {
        out = out.replace(/<pic:spPr([^>]*)\/>/, `<pic:spPr$1>${xfrm}</pic:spPr>`)
      } else if (/<pic:spPr[^>]*>/.test(out)) {
        out = out.replace(/<pic:spPr([^>]*)>/, `<pic:spPr$1>${xfrm}`)
      } else {
        out = out.replace(
          '</pic:pic>',
  ...
  if (patch.crop !== undefined) {
    const sourceRect = patch.crop
      ? `<a:srcRect l="${Math.round(patch.crop.l * 100_000)}" t="${Math.round(patch.crop.t * 100_000)}" r="${Math.round(patch.crop.r * 100_000)}" b="${Math.round(patch.crop.b * 100_000)}"/>`
      : ''
    if (/<a:srcRect\b[^>]*\/>/.test(out)) {
      out = out.replace(/<a:srcRect\b[^>]*\/>/, sourceRect)
    } else if (sourceRect) {
      out = out.replace(/(<a:blip\b[^>]*\/>|<\/a:blip>)/, `$1${sourceRect}`)
    }
  }
...
export function applyImageWrap(
  ...
  const keepKind = wrap.startsWith('tight-')
    ...
    : wrap.startsWith('through-')
      ? 'wp:wrapThrough'
      : null
  const offsetWrapText = wrap.endsWith('-right') ? 'left' : 'right'
  const preserveSideAtOffset = (element: string): string => {
    if (posOffset === undefined || !isSide) return element
    return /wrapText="[^"]*"/.test(element)
      ? element.replace(/wrapText="[^"]*"/, `wrapText="${offsetWrapText}"`)
      : element.replace(/^<[^\s>]+/, (tag) => `${tag} wrapText="${offsetWrapText}"`)
  }
  const wrapElement =
    keepKind && existingWrap?.startsWith(`<${keepKind}`)
      ? preserveSideAtOffset(existingWrap)
      : isSide
        ? `<wp:wrapSquare wrapText="${posOffset === undefined ? 'bothSides' : offsetWrapText}"/>`
        : wrap === 'topBottom'
          ? '<wp:wrapTopAndBottom/>'
          : '<wp:wrapNone/>'
  const anchorOpen =
