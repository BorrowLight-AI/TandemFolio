...
export interface SaveOptions {
  ...
  titlePg?: boolean
  /**
   * Per-section header/footer edits (non-last sections of multi-section docs).
   * lastBlockIndex locates the section's break paragraph: if the section
...
export async function saveDocx(
  ...
  const embedImage = (image: NewImage): string => {
    ...
    const xml =
      ...
      `<pic:blipFill><a:blip r:embed="${rId}"/>${image.crop ? `<a:srcRect l="${Math.round(image.crop.l * 100_000)}" t="${Math.round(image.crop.t * 100_000)}" r="${Math.round(image.crop.r * 100_000)}" b="${Math.round(image.crop.b * 100_000)}"/>` : ''}<a:stretch><a:fillRect/></a:stretch></pic:blipFill>` +
  ...
  for (const edit of options.sectionHf ?? []) {
    ...
    const existing =
      ...
      (variant === 'default' ? refs.find((r) => !/w:type="/.test(r)) : undefined)
    const rId = existing ? /r:id="([^"]+)"/.exec(existing)?.[1] : undefined
    const target = rId ? relTargets.get(rId) : undefined
    if (target) {
      const path = target.startsWith('/') ? target.slice(1) : `word/${target}`
  ...
  for (const block of parsed.blocks) {
    if (block.hidden && block.docxIndex !== null) {
      ...
      if (xml.includes('<w:sectPr')) {
        ...
        if (options.titlePg !== undefined) xml = applySectionTitlePage(xml, options.titlePg)
