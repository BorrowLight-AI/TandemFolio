...
function imageMeta(xml: string): ImageMeta {
  ...
  if (anchor) {
    if (/behindDoc="1"/.test(anchor)) meta.imageWrap = 'behind'
      ...
      const hasHorizontalAlign = /<wp:positionH[^>]*>[\s\S]*?<wp:align>/.test(xml)
      const alignRight =
        /<wp:positionH[^>]*>(?:(?!<\/wp:positionH>)[\s\S])*?<wp:align>right<\/wp:align>/.test(xml)
      const offsetWrapText = /<wp:wrap(?:Square|Tight|Through)[^>]*wrapText="(left|right)"/.exec(
        xml,
      )?.[1]
      const side =
        alignRight || (!hasHorizontalAlign && offsetWrapText === 'left') ? 'right' : 'left'
      meta.imageWrap =
        kind === 'Tight'
          ? `tight-${side}`
          : kind === 'Through'
