...
describe('sectionHf per-section headers/footers', () => {
  ...
  it('writes an explicit first/even variant on a non-final section', async () => {
    const parsed = await parseDocx(
      await buildDocx({ bodyXml: P('第一节') + sectBreakPara() + P('第二节') }),
    )
    const sections = readSections(parsed)
    const saved = await saveDocx(parsed, visibleBlocks(parsed), {
      sectionHf: [
    ...
    const reparsed = await parseDocx(saved)
    const reopened = readSections(reparsed)
    const rId = reopened[0].headerRefs.first
    expect(rId).toBeDefined()
    expect(reparsed.hfParts?.[rId!]?.text).toContain('第一节首页页眉')
    expect(reopened[0].headerRefs.default).toBeUndefined()
    expect(reopened[1].headerRefs.first).toBeUndefined()
  })
})
