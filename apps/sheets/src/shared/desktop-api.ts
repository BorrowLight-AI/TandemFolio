...
export const workbookPageSetupStateSchema = z
  .object({
    sheetId: z.string().min(1),
    tabColor: z.union([hexColorSchema, z.null()]).optional(),
...
/// Shape-only text/fill fields patch the native drawing shape in place.
export const workbookVisualEditSchema = z
  ...
  .refine(
    ...
    {
      message: 'A visual edit needs a removal, anchor, shape text, or shape fill.',
    },
  )
...
export const workbookTableAddSchema = z
  .object({
    ...
    area: cellAreaSchema,
    /// Final unique table name assigned by the renderer when omitted.
      ...
      .array(
        z
          .object({
            ...
            pivotPath: z
              .string()
              .regex(/^xl\/pivotTables\/[A-Za-z0-9._-]+\.xml$/)
              .optional(),
            memberHiddenItems: z
              .array(
                z
                  .object({
                    field: z.number().int().min(0).max(199),
                    hiddenItems: z.array(z.number().int().min(0).max(100_000)).max(100_000),
                  })
                  .strict(),
              )
              .max(200)
...
/// Reads a user-named local image for user-requested insertion; the main
...
/// A rendered print job: the renderer lays the sheet out as HTML and the
/// browser host turns it into a PDF or exposes a browser print fallback.
