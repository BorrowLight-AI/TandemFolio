...
function setTabColor(xml: string, color: string | null): string {
  const existing = /<tabColor\b[^>]*\/?>(?:[\s\S]*?<\/tabColor>)?/.exec(xml)
  if (existing) {
    const replacement = color === null ? '' : `<tabColor rgb="FF${color.slice(1)}"/>`
    return xml.slice(0, existing.index) + replacement + xml.slice(existing.index + existing[0].length)
  }
  if (color === null) return xml
  const tabColor = `<tabColor rgb="FF${color.slice(1)}"/>`
  const sheetPr = /<sheetPr\b[^>]*(\/?)>/.exec(xml)
  if (sheetPr) {
    if (sheetPr[1] === '/') {
      const expanded = `${sheetPr[0].slice(0, -2)}>${tabColor}</sheetPr>`
      return xml.slice(0, sheetPr.index) + expanded + xml.slice(sheetPr.index + sheetPr[0].length)
    }
    const at = sheetPr.index + sheetPr[0].length
    return xml.slice(0, at) + tabColor + xml.slice(at)
  }
  const worksheetOpen = /<worksheet\b[^>]*>/.exec(xml)
  if (!worksheetOpen) throw new PageSetupError('Worksheet has no root element.')
  const at = worksheetOpen.index + worksheetOpen[0].length
  return xml.slice(0, at) + `<sheetPr>${tabColor}</sheetPr>` + xml.slice(at)
}
...
/// Rewrites only the odd sections; page-variant fields and attributes stay intact.
...
export function applyPageSetupState(worksheetXml: string, state: SheetPageSetupState): string {
  ...
  if (state.tabColor !== undefined) xml = setTabColor(xml, state.tabColor)
