...
/// Expand a shared formula master for a follower cell. Relative references
/// move by the follower offset while `$`-anchored components stay fixed.
/// Returning null prevents a bare shared tag when a translation leaves the
/// worksheet bounds.
...
function translateSharedToken(token: string, rowDelta: number, columnDelta: number): string | null {
  const parts = token.split(':').map((part) => {
    ...
    if (wholeColumn) {
      ...
      return column === null ? null : `${wholeColumn[1]}${columnToLetters(column)}`
    }
    ...
    if (wholeRow) {
      ...
      return row === null ? null : `${wholeRow[1]}${row + 1}`
    }
    return part
  })
