...
function dataBarRule(rule: Record<string, unknown>, priority: number, stopIfTrue: boolean): string {
  ...
  if (config.isGradient === false) {
    throw new CfEditError('A solid data bar requires an extended x14 gradient setting.')
  }
  const positiveColor = requireColor(config.positiveColor ?? '#638EC6', 'data bar')
  if (
    config.nativeColor !== undefined &&
    toArgb(requireColor(config.nativeColor, 'negative data bar')) !== toArgb(positiveColor)
  ) {
    throw new CfEditError('A distinct negative data-bar color requires extended x14 formatting.')
  }
  ...
  return (
    ...
    `<color rgb="${toArgb(positiveColor)}"/></dataBar></cfRule>`
  )
}
