...
export interface PivotRefreshUpdate {
  ...
  /// Stable pivotTable part path. Required for member-state updates because
  /// multiple PivotTables may share one cache definition.
  readonly pivotPath?: string | undefined
  ...
  /// Exact hidden item indices for fields changed by slicer/timeline/member
  /// filtering. An empty hiddenItems array explicitly clears that field.
  readonly memberHiddenItems?:
    | readonly {
...
export async function applyPivotLayoutExpansions(
  ...
  for (const update of updates) {
    if (
      update.pivotPath !== undefined &&
      !/^xl\/pivotTables\/[A-Za-z0-9._-]+\.xml$/.test(update.pivotPath)
    ) {
      throw new PivotExpandError(`Invalid pivot table path "${update.pivotPath}".`)
    }
    const pivotTablePath =
      update.pivotPath ?? (await findPivotTablePathForCache(pkg, update.cachePath))
    if (!pivotTablePath) {
    ...
    // Same size with neither layout nor member-state changes: no-op.
    if (
      sameArea &&
      update.relayout === undefined &&
      (update.memberHiddenItems === undefined || update.memberHiddenItems.length === 0)
    ) {
      continue
    }
    ...
    if (update.relayout === undefined) {
      let updatedTableXml = sameArea
        ? pivotTableXml
        : updateLocationRef(pivotTableXml, update.newOutputRef)
      if (update.memberHiddenItems !== undefined && update.memberHiddenItems.length > 0) {
        updatedTableXml = updatePivotMemberHiddenItems(
          updatedTableXml,
          update.memberHiddenItems,
        )
      }
...
function updatePivotMemberHiddenItems(
  pivotTableXml: string,
  updates: readonly { readonly field: number; readonly hiddenItems: readonly number[] }[],
): string {
  const pivotFieldsMatch = /<pivotFields\b[^>]*>([\s\S]*?)<\/pivotFields>/.exec(pivotTableXml)
  if (!pivotFieldsMatch || pivotFieldsMatch.index === undefined) {
    throw new PivotExpandError('Pivot member filtering requires a pivotFields block.')
  }
  const fields = pivotFieldsMatch[1]!.match(/<pivotField\b[^>]*(?:\/>|>[\s\S]*?<\/pivotField>)/g) ?? []
  const seen = new Set<number>()
  for (const update of updates) {
    if (!Number.isSafeInteger(update.field) || update.field < 0 || update.field >= fields.length) {
      throw new PivotExpandError(`Pivot member field ${update.field} is out of range.`)
    }
    if (seen.has(update.field)) {
      throw new PivotExpandError(`Pivot member field ${update.field} is repeated.`)
    }
    seen.add(update.field)
    const fieldXml = fields[update.field]!
    const itemsMatch = /<items\b[^>]*>([\s\S]*?)<\/items>/.exec(fieldXml)
    if (!itemsMatch || itemsMatch.index === undefined) {
      throw new PivotExpandError(`Pivot member field ${update.field} has no items block.`)
    }
    const items = itemsMatch[1]!.match(/<item\b[^>]*(?:\/>|>[\s\S]*?<\/item>)/g) ?? []
    const hidden = new Set(update.hiddenItems)
    if (
      hidden.size !== update.hiddenItems.length ||
      [...hidden].some((member) => !Number.isSafeInteger(member) || member < 0 || member >= items.length)
    ) {
      throw new PivotExpandError(`Pivot member field ${update.field} has an invalid hidden item.`)
    }
    const nextItems = items
      .map((item, index) => {
        const withoutHidden = item.replace(/\s+h="[^"]*"/g, '')
        if (!hidden.has(index)) return withoutHidden
        return withoutHidden.replace(/\s*(\/?>)/, ' h="1"$1')
      })
      .join('')
    fields[update.field] =
      fieldXml.slice(0, itemsMatch.index) +
      itemsMatch[0].replace(itemsMatch[1]!, nextItems) +
      fieldXml.slice(itemsMatch.index + itemsMatch[0].length)
  }
  const nextFields = fields.join('')
  const pivotFieldsXml = pivotFieldsMatch[0].replace(pivotFieldsMatch[1]!, nextFields)
  return (
    pivotTableXml.slice(0, pivotFieldsMatch.index) +
    pivotFieldsXml +
    pivotTableXml.slice(pivotFieldsMatch.index + pivotFieldsMatch[0].length)
  )
}
