...
export async function readSheetNotes(
  worksheetPath: string,
  relsXml: string | null,
  readText: (path: string) => Promise<string | null>,
): Promise<SheetNote[]> {
  if (relsXml === null) return []
  const target = relTarget(relsXml, COMMENTS_REL_TYPE)
  if (target === null) return []
  const commentsXml = await readText(resolveRelTarget(worksheetPath, target))
  if (commentsXml === null) return []
  const authorsBody = /<authors\b[^>]*>([\s\S]*?)<\/authors>/.exec(commentsXml)?.[1] ?? ''
  const authors = [...authorsBody.matchAll(/<author\b[^>]*>([\s\S]*?)<\/author>/g)].map((match) =>
    decodeXml((match[1] ?? '').replace(/<[^>]+>/g, '')),
  )
  return [...commentsXml.matchAll(/<comment\b([^>]*)>([\s\S]*?)<\/comment>/g)].flatMap((match) => {
    const attributes = match[1] ?? ''
    const ref = /\bref="([^"]+)"/.exec(attributes)?.[1]?.replace(/\$/g, '')
    const authorId = Number(/\bauthorId="([0-9]+)"/.exec(attributes)?.[1] ?? -1)
    if (!ref || !/^[A-Z]{1,3}[1-9][0-9]{0,6}$/.test(ref)) return []
    let column = 0
    const address = /^([A-Z]+)([1-9][0-9]*)$/.exec(ref)!
    for (const character of address[1]!) column = column * 26 + character.charCodeAt(0) - 64
