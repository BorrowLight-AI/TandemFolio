...
function patchShapeText(anchorXml: string, text: string): string {
  const shape = /<xdr:sp\b[\s\S]*?<\/xdr:sp>/.exec(anchorXml)
  if (!shape) throw new VisualEditError('Only native drawing shapes support text edits.')
  const txBody =
    '<xdr:txBody><a:bodyPr wrap="square" rtlCol="0"/><a:lstStyle/>' +
    `<a:p><a:r><a:t>${escapeXmlText(text)}</a:t></a:r></a:p></xdr:txBody>`
  const patched = /<xdr:txBody\b[\s\S]*?<\/xdr:txBody>/.test(shape[0])
    ? shape[0].replace(/<xdr:txBody\b[\s\S]*?<\/xdr:txBody>/, () => txBody)
    : shape[0].replace('</xdr:sp>', () => `${txBody}</xdr:sp>`)
  return anchorXml.replace(shape[0], () => patched)
}

function patchShapeFill(anchorXml: string, color: string): string {
  const shape = /<xdr:sp\b[\s\S]*?<\/xdr:sp>/.exec(anchorXml)
  if (!shape) throw new VisualEditError('Only native drawing shapes support fill edits.')
  const properties = /<xdr:spPr\b[^>]*>([\s\S]*?)<\/xdr:spPr>/.exec(shape[0])
  if (!properties || properties[1] === undefined) {
    throw new VisualEditError('The drawing shape has no shape properties.')
  }
  const fill = `<a:solidFill><a:srgbClr val="${color.slice(1).toUpperCase()}"/></a:solidFill>`
  const fillPattern =
    /<a:solidFill\b[\s\S]*?<\/a:solidFill>|<a:gradFill\b[\s\S]*?<\/a:gradFill>|<a:pattFill\b[\s\S]*?<\/a:pattFill>|<a:noFill\s*\/>/
  const inner = properties[1]
  const nextInner = fillPattern.test(inner)
    ? inner.replace(fillPattern, () => fill)
    : inner.replace(
        /(<a:prstGeom\b[\s\S]*?<\/a:prstGeom>)/,
        (_all, geometry: string) => `${geometry}${fill}`,
      )
  if (nextInner === inner && !fillPattern.test(inner)) {
    throw new VisualEditError('The drawing shape has no supported geometry anchor for its fill.')
  }
  const patchedProperties = properties[0].replace(inner, () => nextInner)
  return anchorXml.replace(shape[0], () => shape[0].replace(properties[0], patchedProperties))
}
...
function applyOneEdit(xml: string, edit: WorkbookVisualEdit, removedChartRelIds: string[]): string {
  ...
  if (edit.text !== undefined) patched = patchShapeText(patched, edit.text)
  if (edit.fillColor !== undefined) patched = patchShapeFill(patched, edit.fillColor)
  const anchor = edit.anchor
  if (!anchor) {
    if (patched === anchorXml) {
      throw new VisualEditError('A visual edit needs a removal, anchor, shape text, or shape fill.')
    }
  ...
  const withFrom = patched.replace(/<xdr:from>[\s\S]*?<\/xdr:from>/, () => from)
  if (withFrom === patched && !patched.includes('<xdr:from>')) {
