...
export interface PageSetupJournalState {
  /// Worksheet tab color as #RRGGBB, or null to remove `<tabColor>`.
...
export function recordPivotRefreshUpdate(
  ...
  memberState?: {
    ...
    readonly memberHiddenItems: readonly {
      readonly field: number
      readonly hiddenItems: readonly number[]
    }[]
  },
...
export function recordVisualEdit(
  ...
  const text = changes.text ?? previous?.text
...
function mergeCellIntoEntry(
  ...
  if (formulaText) {
    ...
    if (isCalculationResult) {
      // Keep the journaled formula and its latest cached result. Readers that
      // do not calculate formulas still need the saved <v> to match the
      // value shown by the mounted community renderer.
      if (
        raw === null ||
        typeof raw === 'string' ||
        typeof raw === 'boolean' ||
