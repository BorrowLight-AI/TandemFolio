...
export interface SaveActionSuccess {
  readonly ok: true
  readonly fileName: string
}

export interface SaveActionFailure {
  readonly ok: false
  readonly message: string
}

export interface SaveRecoverySuccess {
  readonly ok: true
  readonly fileName: string
  readonly data: ArrayBuffer
}

export type SaveActionResult = SaveActionSuccess | SaveRecoverySuccess | SaveActionFailure
...
export async function handleSave(
  ...
  const edits = toSaveEdits(state.editJournal).map((edit) => {
    if (edit.formula === undefined) return edit
    const live = workbook
      ?.getSheetBySheetId(edit.sheetId)
  ...
  const pivotRefreshUpdates = [...state.editJournal.pivotRefreshUpdates].map(
    ([cachePath, update]) => ({
      ...
      ...(update.memberHiddenItems === undefined
        ? {}
        : {
            memberHiddenItems: update.memberHiddenItems.map(({ field, hiddenItems }) => ({
              field,
  ...
  if (mode === 'recovery') {
    ...
    const recovery = await window.desktopApi
      .writeWorkbookRecovery(payload)
      .catch(() => ({ ok: false as const }))
    return recovery.ok ? recovery : undefined
  }
  try {
    ...
    if (result.canceled) {
      const canceled = t('appSaveCanceled')
      ctx.setMessage(canceled)
    ...
    try {
      ...
      ctx.setMessage(saved)
      if (!quiet) showToast(saved)
      return { ok: true, fileName: second.file.name }
    } catch (error: unknown) {
      if (ctx.lazyWorkbookRef.current !== state) return
      ctx.openLazyWorkbook(result.file)
