...
import type { AddPivotOperation } from '../domain/workbook-dsl'
...
export function updatePivotLayout(
  ...
  if (!state.formulaMode || !state.flags.preloadComplete) {
    throw new Error('xlsx.pivot.update requires a fully loaded workbook.')
  }
  const targetMeta = state.file.sheets.find((sheet) =>
    sheet.pivotTables.some((pivot) => pivot.path === input.pivotId),
  )
  const pivot = targetMeta?.pivotTables.find((entry) => entry.path === input.pivotId)
  if (!targetMeta || !pivot) throw new Error(`Unknown XLSX pivot: ${input.pivotId}`)
  if (pivot.cachePath === null) throw new Error('The XLSX pivot has no editable cache definition.')
  const definition = state.pivotDefinitions.get(pivot.path)
  if (!definition) throw new Error('The XLSX pivot definition is not loaded.')
  if (
    definition.unsupported.length > 0 ||
    definition.fields.some((field) => field.grouping !== undefined || field.formula !== undefined) ||
    definition.filters.length > 0
  ) {
    throw new Error('The XLSX pivot contains unsupported layout features and cannot be replaced.')
  }
  const sourceMeta = state.file.sheets.find((sheet) => sheet.name === definition.sourceSheet)
  if (!sourceMeta) throw new Error(`Unknown XLSX pivot source sheet: ${definition.sourceSheet}`)
  const bounds = parseRange(pivot.outputRef)
  applyAiPivotAdd(
    runtime,
    state,
    {
      op: 'add_pivot',
      sheetId: sourceMeta.id,
      ...(targetMeta.id === sourceMeta.id ? {} : { targetSheetId: targetMeta.id }),
      sourceRange: definition.sourceRef,
      targetCell: input.targetCell,
      rowFields: [...input.rowFields],
      ...(input.columnField === undefined ? {} : { columnField: [...input.columnField] }),
      ...(input.pageFields === undefined ? {} : { pageFields: [...input.pageFields] }),
      values: input.values,
    },
    {
      pivotPath: pivot.path,
...
export function setPivotMemberFilter(
  ctx: PivotActionContext,
  input: {
    readonly pivotId: string
  ...
  const sheetMeta = state.file.sheets.find((sheet) =>
    sheet.pivotTables.some((pivot) => pivot.path === input.pivotId),
  )
  const pivot = sheetMeta?.pivotTables.find((entry) => entry.path === input.pivotId)
  if (!sheetMeta || !pivot) throw new Error(`Unknown XLSX pivot: ${input.pivotId}`)
  const definition = state.pivotDefinitions.get(pivot.path)
  if (!definition) throw new Error('The XLSX pivot definition is not loaded.')
  const field = definition.fields.findIndex((candidate) => candidate.name === input.field)
  if (field < 0) throw new Error(`Unknown XLSX pivot field: ${input.field}`)
  ...
  if (!dimensionFields.has(field)) {
    throw new Error(`XLSX pivot field "${input.field}" is not a filterable dimension.`)
  }
  if (definition.filters.some((filter) => filter.field === field)) {
    throw new Error(`XLSX pivot field "${input.field}" already has a value or label filter.`)
  }
  const valueKey = (value: string | number | boolean | null): string =>
    `${typeof value}:${String(value)}`
  const memberValues = new Map<string, number>()
  for (const [member, item] of (definition.fieldItems[field] ?? []).entries()) {
    if (item.x === null) continue
    const value = definition.fields[field]?.sharedItems[item.x]
    if (value !== undefined) memberValues.set(valueKey(value), member)
  }
  if (memberValues.size === 0) {
    throw new Error(`XLSX pivot field "${input.field}" has no filterable members.`)
  }
  const selectedMembers =
    input.selectedValues === null
      ? null
      : input.selectedValues.map((value) => {
          const member = memberValues.get(valueKey(value))
          if (member === undefined) {
            throw new Error(`Unknown XLSX pivot member for "${input.field}": ${String(value)}`)
          }
          return member
        })
  const failure = applySlicerSelection(
    ctx,
    { sheetId: sheetMeta.id, pivotPath: pivot.path, field },
    selectedMembers,
  )
  if (failure !== null) throw new Error(failure)
  return selectedMembers?.length ?? memberValues.size
}
...
export function applySlicerSelection(
  ...
  try {
    ...
    if (pivot.cachePath !== null) {
      // Save both the resized output ref and exact pivotField hidden-item
      // snapshot; refreshOnLoad then rebuilds the cache without losing the
      // explicit member filter.
      recordPivotRefreshUpdate(
        ...
        {
          pivotPath: pivot.path,
          memberHiddenItems: next.fieldItems.flatMap((items, field) =>
            field === slicer.field
              ? [
                  {
                    field,
                    hiddenItems: items.flatMap((item, member) => (item.hidden ? [member] : [])),
                  },
                ]
              : [],
          ),
