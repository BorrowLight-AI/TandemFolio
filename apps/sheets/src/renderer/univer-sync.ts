...
/** Applies the file-side worksheet protection state shared by Ribbon and registry callers. */
export function applyWorkbookSheetProtection(
  state: Pick<LazyWorkbookState, 'editJournal' | 'sheetProtections'>,
  sheetId: string,
  nextProtected: boolean,
): string | null {
...
/** Applies file-only cell protection through the neutral OOXML style journal. */
export function applyWorkbookCellProtection(
  state: Pick<LazyWorkbookState, 'editJournal'>,
  worksheet: UniverWorksheet,
  range: ReturnType<UniverWorksheet['getRange']>,
  protection: { readonly locked?: boolean; readonly hidden?: boolean },
): void {
  const sheetId = worksheet.getSheetId()
  if (isSheetRemoved(state.editJournal, sheetId)) throw new Error(`Unknown sheet: ${sheetId}`)
  const delta: WorkbookStyleEdit = {}
  if (protection.locked !== undefined) delta.protectionLocked = protection.locked
  if (protection.hidden !== undefined) delta.protectionHidden = protection.hidden
...
export type WorkbookAggregateFunction = 'SUM' | 'AVERAGE' | 'COUNT' | 'MAX' | 'MIN'

/** Applies one final scalar through the native cell write/selection route. */
export function applyWorkbookCellValue(
  range: ReturnType<UniverWorksheet['getRange']>,
  value: string | number | boolean | null,
): void {
  range.setValue(value === null ? ({ v: null } satisfies ICellData) : value)
  range.activate()
}

/** Applies one explicit formula through the same native cell/selection route. */
export function applyWorkbookCellFormula(
  range: ReturnType<UniverWorksheet['getRange']>,
  formula: string,
): void {
...
/** Applies one resolved cell-data matrix through the native range write/selection route. */
export function applyWorkbookCellMatrix(
  range: ReturnType<UniverWorksheet['getRange']>,
  values: ICellData[][],
): void {
...
export type WorkbookFlashFillBlocked =
  'needs_left' | 'needs_rows' | 'streaming' | 'needs_examples' | 'no_pattern' | 'nothing_to_fill'

export type WorkbookFlashFillResult =
  | { readonly filled: number; readonly targetRange: string }
  | { readonly blocked: WorkbookFlashFillBlocked }

export type WorkbookTextToColumnsDelimiter = 'tab' | 'comma' | 'semicolon' | 'space'

/** Applies one explicit point height through Univer's native row command and undo journal. */
export function applyWorkbookRowHeight(
  worksheet: UniverWorksheet,
  startRow: number,
  count: number,
  heightPoints: number,
): void {
  worksheet.setRowHeightsForced(startRow, count, Math.round((heightPoints * 96) / 72))
}

/** Applies one explicit character width through Univer's native column command and undo journal. */
export function applyWorkbookColumnWidth(
  worksheet: UniverWorksheet,
  startColumn: number,
  count: number,
  widthCharacters: number,
): number {
  const widthPixels = Math.round(characterWidthToPixels(widthCharacters))
  worksheet.setColumnWidths(startColumn, count, widthPixels)
  return pixelsToCharacterWidth(widthPixels)
}

/** Applies an explicit freeze split, using zero rows and columns as the unfreeze state. */
export function applyWorkbookFreeze(
  worksheet: UniverWorksheet,
  frozenRows: number,
  ...
  worksheet.setFreeze({
    startRow: frozenRows === 0 ? -1 : frozenRows,
    startColumn: frozenColumns === 0 ? -1 : frozenColumns,
    xSplit: frozenColumns,
    ySplit: frozenRows,
  })
}

/** Applies an explicit persisted worksheet gridline visibility. */
export function applyWorkbookGridlineVisibility(
  worksheet: UniverWorksheet,
  visible: boolean,
): void {
  worksheet.setHiddenGridlines(!visible)
}

const workbookTextToColumnsDelimiterFlag: Record<WorkbookTextToColumnsDelimiter, 1 | 2 | 4 | 8> = {
  tab: 1,
  comma: 2,
  semicolon: 4,
  space: 8,
...
export function workbookTextToColumnsDelimiterFromFlag(
  flag: string,
): WorkbookTextToColumnsDelimiter | null {
  if (flag === '1') return 'tab'
  if (flag === '2') return 'comma'
  if (flag === '4') return 'semicolon'
  if (flag === '8') return 'space'
  return null
}

/** Executes the retained Text to Columns command for both Ribbon and registry callers. */
export function applyWorkbookTextToColumns(
  runtime: UniverRuntime,
  worksheet: UniverWorksheet,
  range: ReturnType<UniverWorksheet['getRange']>,
  delimiter: WorkbookTextToColumnsDelimiter,
): Promise<boolean> {
  const workbook = runtime.univerAPI.getActiveWorkbook()
  if (!workbook) return Promise.resolve(false)
  return runtime.univerAPI.executeCommand('sheet.command.split-text-to-columns', {
    ...
    delimiter: workbookTextToColumnsDelimiterFlag[delimiter],
  })
}

export function streamedWorkbookRectangleAvailable(
  state: Pick<LazyWorkbookState, 'file' | 'loadedRanges' | 'flags'> | null,
  sheetId: string,
  startRow: number,
  endRow: number,
  startColumn: number,
  ...
  const originalEndRow = Math.min(endRow, sheet.rowCount - 1)
  const originalEndColumn = Math.min(endColumn, sheet.columnCount - 1)
  if (startRow > originalEndRow || startColumn > originalEndColumn) return true
  const loaded = state.loadedRanges.get(sheetId)
  return (
    loaded !== undefined &&
    startRow >= loaded.startRow &&
    originalEndRow <= loaded.endRow &&
    startColumn >= loaded.startColumn &&
    originalEndColumn <= loaded.endColumn
  )
}

/** Applies Flash Fill through one facade write shared by retained Ribbon and registry callers. */
export function applyWorkbookFlashFill(
  state: Pick<LazyWorkbookState, 'file' | 'loadedRanges' | 'flags'> | null,
  worksheet: UniverWorksheet,
  range: ReturnType<UniverWorksheet['getRange']>,
): WorkbookFlashFillResult {
  const startRow = range.getRow()
  const targetColumn = range.getColumn()
  if (targetColumn === 0) return { blocked: 'needs_left' }

  const sheetId = worksheet.getSheetId()
  let endRow = startRow + range.getHeight() - 1
  if (range.getHeight() === 1) {
    const probeRows = Math.min(1_000, worksheet.getMaxRows() - startRow)
    if (
      !streamedWorkbookRectangleAvailable(
        state,
        sheetId,
        startRow,
        startRow + probeRows - 1,
        targetColumn - 1,
        targetColumn - 1,
      )
    ) {
      return { blocked: 'streaming' }
    }
    const probe = worksheet.getRange(startRow, targetColumn - 1, probeRows, 1).getValues() as (
      string | number | boolean | null | undefined
    )[][]
    let last = startRow
    for (const [offset, row] of probe.entries()) {
      const value = row[0]
  ...
  if (rowCount < 2) return { blocked: 'needs_rows' }
  const firstSource = Math.max(0, targetColumn - 6)
  if (
    !streamedWorkbookRectangleAvailable(state, sheetId, startRow, endRow, firstSource, targetColumn)
  ) {
    return { blocked: 'streaming' }
  }
  ...
  const sourceOf = (row: readonly (string | number | boolean | null | undefined)[]): string[] =>
    row.slice(0, -1).map((value) => String(value ?? ''))
  ...
  if (examples.length === 0) return { blocked: 'needs_examples' }
  const template = inferFlashFillTemplate(examples)
  if (!template) return { blocked: 'no_pattern' }

  let filled = 0
  const matrix: (string | number | boolean)[][] = grid.map((row) => {
    const current = row[row.length - 1]
    if (current !== null && current !== undefined && String(current) !== '') return [current]
    const source = sourceOf(row)
    if (source.every((value) => value.trim() === '')) return ['']
    filled += 1
    return [applyFlashFillTemplate(template, source)]
  })
  if (filled === 0) return { blocked: 'nothing_to_fill' }

  worksheet.getRange(startRow, targetColumn, rowCount, 1).setValues(matrix)
  return {
    filled,
    targetRange: `${columnLabel(targetColumn)}${startRow + 1}:${columnLabel(targetColumn)}${
      startRow + rowCount
    }`,
  }
}

/** Applies the AutoSum-family write shared by the retained Ribbon and registry callers. */
export function applyWorkbookAggregateFormula(
  state: Pick<LazyWorkbookState, 'file' | 'loadedRanges' | 'flags'> | null,
  worksheet: UniverWorksheet,
  range: ReturnType<UniverWorksheet['getRange']>,
  aggregate: WorkbookAggregateFunction,
): { readonly inserted: number; readonly targetRange: string } | { readonly blocked: 'streaming' } {
  const startRow = range.getRow()
  ...
  if (state && !state.flags.preloadComplete) {
    ...
    if (targetInData && !targetLoaded) return { blocked: 'streaming' }
  }
  for (let column = startColumn; column <= endColumn; column += 1) {
    const letter = columnLabel(column)
    worksheet
      .getRange(lastRow + 1, column)
      .setFormula(`=${aggregate}(${letter}${startRow + 1}:${letter}${lastRow + 1})`)
  }
  return {
    inserted: range.getWidth(),
    targetRange: `${columnLabel(startColumn)}${lastRow + 2}:${columnLabel(endColumn)}${lastRow + 2}`,
  }
}

/// Same journal write + link styling as the Insert Link menu action.
export function applyWorkbookHyperlink(
  state: Pick<LazyWorkbookState, 'editJournal' | 'hyperlinkTargets'>,
  worksheet: UniverWorksheet,
  input: { readonly address: string; readonly target: string | null },
): void {
  const sheetId = worksheet.getSheetId()
  const { row, column } = parseAddress(input.address)
  if (input.target === null) {
    ...
    worksheet
      ...
      .setValue({ s: { ul: null, cl: null } } as unknown as ICellData)
    return
  }
  ...
  worksheet.getRange(input.address).setValue({
...
/// Renderer-owned state changes enter Univer's own undo stack as a custom
/// mutation pair, so ⌘Z interleaves them correctly with native cell edits.
export function pushWorkbookUndo(runtime: UniverRuntime, step: WorkbookUndoStep): void {
  ...
  if (!workbookUndoRuntimes.has(runtime)) {
    workbookUndoRuntimes.add(runtime)
    injector
      .get<{
      ...
      .registerCommand({
        id: WORKBOOK_UNDO_COMMAND_ID,
        type: CommandType.MUTATION,
        handler: (_accessor, params) => {
          const entry = params ? workbookUndoRegistry.get(params.token) : undefined
  ...
  const token = ++workbookUndoSequence
  workbookUndoRegistry.set(token, step)
  injector
    .get<{
      pushUndoRedo(item: {
        unitID: string
    ...
    .pushUndoRedo({
      unitID: unitId,
      undoMutations: [{ id: WORKBOOK_UNDO_COMMAND_ID, params: { token, direction: 'undo' } }],
      redoMutations: [{ id: WORKBOOK_UNDO_COMMAND_ID, params: { token, direction: 'redo' } }],
    })
}
