...
export type WorkbookPivotChartKind = 'column' | 'bar' | 'line' | 'pie' | 'doughnut' | 'radar'

/** Shared exact-Pivot action for the PivotChart gallery and registry tracer. */
export async function insertAiPivotChartVisual(
  ctx: VisualActionContext,
  pivotId: string,
  chartKind: WorkbookPivotChartKind,
  ...
  const sheetMeta = state.file.sheets.find((sheet) =>
    sheet.pivotTables.some((pivot) => pivot.path === pivotId),
  )
  const pivot = sheetMeta?.pivotTables.find((candidate) => candidate.path === pivotId)
  if (!workbook || !sheetMeta || !pivot) throw new Error(`Unknown XLSX pivot: ${pivotId}`)
  const worksheet = workbook.getSheetBySheetId(sheetMeta.id)
  if (!worksheet || isSheetRemoved(state.editJournal, sheetMeta.id)) {
    throw new Error(`The XLSX pivot worksheet is unavailable: ${sheetMeta.id}`)
  }
  ...
  const outputValues = await readChartGridValues(state, runtime, sheetMeta.id, pivot.outputRef)
  const chartData = buildPivotChartData(definition, worksheet.getSheetName(), bounds, outputValues)
  if (chartData.series.length === 0) throw new Error('The XLSX PivotTable has no chartable data.')
  const visual: WorkbookVisualObject = {
  ...
  pushVisualAddUndo(ctx, runtime, state, visual)
  ctx.setPendingEdits(journalSize(state.editJournal))
  queueCtxVisualInstall(ctx, runtime, sheetMeta.id)
  return { chartId: visual.id, truncated: chartData.truncated }
}

/// PivotChart gallery adapter: resolves the selected PivotTable, then calls
/// the same exact-ID action as `xlsx.pivot.add_chart`.
export async function handleInsertPivotChart(
  ...
  try {
    const result = await insertAiPivotChartVisual(
      ctx,
      found.pivot.path,
      chartKind as WorkbookPivotChartKind,
    )
    ctx.setMessage(
      result.truncated ? t('appPivotChartInsertedTruncated') : t('appPivotChartInserted'),
    )
  } catch (error) {
