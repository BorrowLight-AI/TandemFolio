...
function createExecutor(name: string): BaseFunction | null {
  const Builtin = functionStatistical.find(([, builtinName]) => builtinName === name)?.[0] as
    | (new (functionName: string) => BaseFunction)
    | undefined
...
export function installIfsEmptySetFix(runtime: UniverRuntime): { dispose(): void } {
  ...
  const originals = executors.map((executor) => functionService.getExecutor(executor.name) ?? undefined)
  functionService.registerExecutors(...executors)
  const timer = setTimeout(() => {
    const stale = executors.filter((executor) => functionService.getExecutor(executor.name) !== executor)
    if (stale.length > 0) functionService.registerExecutors(...stale)
  }, 0)
  return {
    dispose() {
      clearTimeout(timer)
      for (const original of originals) if (original) functionService.registerExecutors(original)
    },
  }
}
