...
export interface LazyWorkbookState {
  ...
  /// Union of row/column default style keys per sheet. Explicit cell xfs
  /// block these keys from bleeding through Univer's composed styles.
  ...
  readonly outline: Map<
    string,
    {
      readonly rows: Map<number, { level: number; collapsed: boolean; hidden?: boolean }>
      readonly cols: Map<number, { level: number; collapsed: boolean; hidden?: boolean }>
    }
  >
