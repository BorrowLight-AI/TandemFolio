...
const DROPDOWN_COMPONENT_KEY = 'tandemfolio-active-data-validation-dropdown'
const INPUT_MESSAGE_COMPONENT_KEY = 'tandemfolio-active-data-validation-input-message'
...
export function openActiveDataValidationDropdown(
  ...
  if (!target || !shouldShowDataValidationDropdown(target.validation)) return
...
function DataValidationDropdownButton({ runtime }: { readonly runtime: UniverRuntime }) {
  return (
    <button
      type="button"
...
function DataValidationInputMessage({ runtime }: { readonly runtime: UniverRuntime }) {
  const message = getDataValidationInputMessage(resolveActiveValidationTarget(runtime)?.validation)
...
/** Install Excel-like active-cell list arrow and validation input message. */
export function installActiveCellDataValidationChrome(runtime: UniverRuntime): {
  ...
  const sync = (force = false): void => {
    ...
    if (!target) return
    if (showDropdown) {
      const height = Math.min(DROPDOWN_SIZE, Math.max(1, rangePixelHeight(target)))
      dropdownFloating =
