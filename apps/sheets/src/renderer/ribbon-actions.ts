...
import {
  ...
  resolveWorkbookFilterToggle,
} from './filter-actions'
import { applyWorkbookFormulaView, formulaViewSheets } from './formula-view'
...
import { applyWorkbookSparklineAdd } from './sparkline-actions'
...
export function handleRibbonCommand(ctx: RibbonCommandContext, command: string): void {
  ...
  switch (command) {
    ...
    case 'filter-toggle':
      {
        const target = resolveWorkbookFilterToggle(runtime)
        if (target) {
          void applyWorkbookFilter(runtime, target.worksheet, target.range, target.enabled)
        }
      }
      return
    case 'filter-clear':
      {
        ...
        if (worksheet && filterRange) {
          void clearWorkbookFilterCriteria(runtime, worksheet, filterRange)
        }
      }
      return
    ...
    case 'insert-checkbox': {
      ...
      applyWorkbookCheckbox(runtime, active, true)
      return
    }
    case 'note-open':
    ...
    case 'freeze-top-row':
      ...
      if (worksheet) {
        applyWorkbookFreeze(worksheet, 1, 0)
        ctx.setMessage(t('appTopRowFrozen'))
      }
      return
    case 'freeze-first-col':
      if (worksheet) {
        applyWorkbookFreeze(worksheet, 0, 1)
    ...
    case 'toggle-gridlines': {
      ...
      applyWorkbookGridlineVisibility(worksheet, !nextHidden)
    ...
    case 'toggle-show-formulas': {
      ...
      applyWorkbookFormulaView(runtime, state, sheetId, next, ctx.setPendingEdits, (step) =>
        pushWorkbookUndo(runtime, step),
      )
    ...
    case 'freeze-here': {
      const active = runtime.univerAPI.getActiveWorkbook()?.getActiveRange()
      if (worksheet && active) {
        applyWorkbookFreeze(worksheet, active.getRow(), active.getColumn())
        ctx.setMessage(t('appFrozenAtSelection'))
      }
      return
    }
    case 'unfreeze':
      if (worksheet) applyWorkbookFreeze(worksheet, 0, 0)
      return
  ...
  if (command.startsWith('row-height:') || command.startsWith('col-width:')) {
    ...
    if (isRow) {
      applyWorkbookRowHeight(worksheet, area.startRow, area.endRow - area.startRow + 1, value)
    } else {
      applyWorkbookColumnWidth(
        worksheet,
        area.startColumn,
  ...
  try {
    ...
    switch (name) {
      ...
      case 'cellprot': {
        ...
        if (argument === 'locked-on') protection.locked = true
        else if (argument === 'locked-off') protection.locked = false
        if (extra === 'hidden-on') protection.hidden = true
        else if (extra === 'hidden-off') protection.hidden = false
        if (Object.keys(protection).length === 0) return
        applyWorkbookCellProtection(state, worksheet, range, protection)
        ctx.setPendingEdits(journalSize(state.editJournal))
      ...
      case 'autofn': {
        ...
        const result = applyWorkbookAggregateFormula(
          ctx.lazyWorkbookRef.current,
          worksheet,
          range,
          argument as 'SUM' | 'AVERAGE' | 'COUNT' | 'MAX' | 'MIN',
        )
      ...
      case 'link-remove': {
        ...
        const target =
          name === 'link-remove' ? null : normalizeLinkTarget(decodeURIComponent(argument))
        if (name !== 'link-remove' && target === null) {
          ctx.setMessage(t('appLinkInvalid'))
          return
        }
        applyWorkbookHyperlink(state, worksheet, {
          address: `${columnLabel(column)}${row + 1}`,
          target,
        })
        ctx.setMessage(name === 'link-remove' ? t('appLinkRemoved') : t('appLinkSaved'))
        ctx.setPendingEdits(journalSize(state.editJournal))
      ...
      case 'flash-fill': {
        if (!worksheet) return
        const state = ctx.lazyWorkbookRef.current
        const result = applyWorkbookFlashFill(state, worksheet, range)
        if ('blocked' in result) {
          if (result.blocked === 'needs_left') ctx.setMessage(t('appFlashFillNeedsLeft'))
          else if (result.blocked === 'needs_rows') ctx.setMessage(t('appFlashFillNeedsRows'))
          else if (result.blocked === 'streaming') ctx.setMessage(t('appAreaStreaming'))
          else if (result.blocked === 'needs_examples') {
            ctx.setMessage(t('appFlashFillNeedsExamples'))
          } else if (result.blocked === 'no_pattern') {
        ...
        ctx.setMessage(t('appFlashFillDone', { count: result.filled }))
        return
      }
      case 'sparkline': {
        if (!worksheet) return
        ...
        const source = worksheet.getRange(startRow, startColumn, rows, range.getWidth())
        const target = worksheet.getRange(startRow, targetColumn, rows, 1)
        ...
        if (!result.ok) {
          ctx.setMessage(
            result.reason === 'source_width' ? t('appSparklineNeedsCols') : t('appCommandFailed'),
          )
          return
        }
        ctx.setMessage(t('appSparklinesInserted', { count: result.count }))
        return
      }
      case 'text-to-columns': {
        ...
        const delimiter = workbookTextToColumnsDelimiterFromFlag(argument)
        if (!delimiter) return
        ctx.setMessage(t('appSplitIntoColumns'))
        void applyWorkbookTextToColumns(runtime, worksheet, range, delimiter)
        return
      }
      case 'border': {
