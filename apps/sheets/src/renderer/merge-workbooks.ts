// Modified by TandemFolio contributors: browser multi-session host and typed staged merge seam.
/**
...
import type { WorkbookFile, WorkbookRangeRequest, WorkbookRangeResult } from '../shared/desktop-api'
import { t } from './i18n/locale'
import type { LazyWorkbookState, UniverRuntime } from './univer-state'
import { characterWidthToPixels, toUniverStyle } from './univer-sync'
...
export interface MergeWorkbookSourceHost {
  openBuffer(data: ArrayBuffer, name: string): Promise<WorkbookFile>
...
async function readSourceSheet(
  ...
  for (let startRow = 0; startRow < sheet.rowCount; startRow += step) {
    ...
    let result = await host.readWorkbookRange({
    ...
    while (
      ...
      result = await host.readWorkbookRange({
        sessionId: file.sessionId,
        sheetId: sheet.id,
        range: { startRow, endRow, startColumn: 0, endColumn: columns - 1 },
      })
  ...
  try {
    ...
    for (const file of sources) {
      for (const sheetMeta of file.sheets) {
        ...
        const imported = await readSourceSheet(deps.host, file, sheetMeta)
    ...
    for (const file of sources) {
      void deps.host.closeWorkbook(file.sessionId).catch(() => {})
    }
  }
}

export interface MergeSourceBytes {
  readonly name: string
  readonly data: ArrayBuffer
}

/** Browser/typed-operation entry point over hydrated XLSX bytes. */
export async function mergeWorkbookBytesIntoCurrent(
  deps: MergeWorkbooksDeps,
  inputs: readonly MergeSourceBytes[],
): Promise<MergeSourcesResult> {
  const state = deps.lazyWorkbookRef.current
  if (state && workbookStructureLocked(state)) {
    throw new Error(t('appMergeWorkbooksLocked'))
  }
  const sources: WorkbookFile[] = []
  try {
    for (const input of inputs) sources.push(await deps.host.openBuffer(input.data, input.name))
  } catch (error) {
    for (const source of sources) void deps.host.closeWorkbook(source.sessionId).catch(() => {})
    throw error
  }
  if (sources.length === 0) throw new Error(t('appMergeWorkbooksFailed'))
  return mergeSourcesIntoCurrent(deps, sources)
}

/** Visible browser picker result routed through the shared byte import core. */
export async function mergeWorkbookFilesIntoCurrent(
  deps: MergeWorkbooksDeps,
  files: readonly File[],
): Promise<MergeSourcesResult> {
  ...
  return mergeWorkbookBytesIntoCurrent(deps, inputs)
}
