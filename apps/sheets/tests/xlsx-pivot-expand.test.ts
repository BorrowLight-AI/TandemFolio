...
describe('applyPivotLayoutExpansions', () => {
  ...
  it('persists an explicit hidden-member snapshot even when the output ref is unchanged', async () => {
    const buf = await buildPivotFixture()
    const update: PivotRefreshUpdate = {
      cachePath: 'xl/pivotCache/pivotCacheDefinition1.xml',
      worksheetPath: 'xl/worksheets/sheet1.xml',
    ...
    expect(updatedTable).toContain('<item x="0"/>')
    expect(updatedTable).toContain('<item x="1" h="1"/>')
    const originalZip = await JSZip.loadAsync(buf)
    const cacheXml =
      plan.replaced.get('xl/pivotCache/pivotCacheDefinition1.xml') ??
      (await originalZip.file('xl/pivotCache/pivotCacheDefinition1.xml')!.async('string'))
    const parsed = parsePivotDefinition(updatedTable!, cacheXml)
    expect(parsed.fieldItems[0]?.map((item) => item.hidden)).toEqual([false, true])
  })
