...
describe('streamed worksheet empty cells', () => {
  it('keeps a literal empty-string cell as an empty string', () => {
    expect(patchedMatrix([{ row: 0, column: 1, value: '' }])[0]?.[1]).toEqual({
      v: '',
      t: CellValueType.STRING,
    })
  })

  it('keeps style-only cells value-less', () => {
    expect(patchedMatrix([{ row: 0, column: 0, value: null }])[0]?.[0]).toEqual({ v: null })
  })

  it('still types non-empty strings', () => {
    expect(patchedMatrix([{ row: 0, column: 2, value: 'x' }])[0]?.[2]).toEqual({
      v: 'x',
      t: CellValueType.STRING,
    })
  })
