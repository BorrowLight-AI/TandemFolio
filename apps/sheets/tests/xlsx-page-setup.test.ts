...
describe('applyPageSetupState', () => {
  it('writes and clears worksheet tab color without dropping other sheet properties', () => {
    const colored = applyPageSetupState(
      '<worksheet><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><sheetData/></worksheet>',
      { sheetName: 'S', tabColor: '#112233' },
    )
    expect(colored).toContain(
      '<sheetPr><tabColor rgb="FF112233"/><pageSetUpPr fitToPage="1"/></sheetPr>',
    )
    const cleared = applyPageSetupState(colored, { sheetName: 'S', tabColor: null })
    expect(cleared).toBe(
      '<worksheet><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><sheetData/></worksheet>',
    )
  })
