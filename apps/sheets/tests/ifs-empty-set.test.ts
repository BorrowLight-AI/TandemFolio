...
describe('fixIfsEmptySetResult', () => {
  ...
  it('passes real numbers and strings through untouched', () => {
  ...
  it('rewrites artifact slots inside an array-criteria expansion', () => {
    ...
    const values = (fixIfsEmptySetResult(outer as never) as ArrayValueObject).getArrayValue()
...
describe('createIfsEmptySetExecutors', () => {
  it('builds MINIFS and MAXIFS wrappers', () => {
    expect(createIfsEmptySetExecutors().map((item) => item.name).sort()).toEqual(['MAXIFS', 'MINIFS'])
  })
})
