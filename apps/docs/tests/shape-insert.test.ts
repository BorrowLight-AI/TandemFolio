...
} from '../src/renderer/editor/object-actions'
...
describe('shape insertion', () => {
  it('routes object-mode Delete through the shared removal kernel', async () => {
    const { editor } = await openBlankDoc()
    insertShape(editor, 'ellipse')
    ...
    expect(editor.state.doc.child(1).attrs.textboxes[0].prst).toBe('ellipse')
    editor.destroy()
  })

  it('removes the sole shape through the shared object action and saves a replacement paragraph', async () => {
    const { editor, parsed } = await openBlankDoc()
    insertShape(editor, 'ellipse')
    editor.view.dispatch(editor.state.tr.delete(0, editor.state.doc.child(0).nodeSize))

    expect(removeDocxObjectAtBlock(editor, { objectBlockIndex: 0 })).toEqual({
      ok: true,
      objectKind: 'shape',
      replacementParagraphInserted: true,
    })
    expect(editor.state.doc.childCount).toBe(1)
    ...
    expect(reparsed.blocks.some((block) => block.type === 'paragraph')).toBe(true)
    expect(reparsed.blocks.some((block) => block.textboxes)).toBe(false)
    expect(editor.commands.undo()).toBe(true)
    expect(editor.state.doc.child(0).attrs.textboxes[0].prst).toBe('ellipse')
    editor.destroy()
  })

  it('styles through the shared object action and preserves fill/outline after save/reopen', async () => {
    const { editor, parsed } = await openBlankDoc()
    insertShape(editor, 'ellipse')

    expect(
      setDocxObjectStyleAtBlock(editor, {
        objectBlockIndex: 1,
        style: { fillHex: 'A1B2C3', borderHex: null },
        fields: ['fillHex', 'borderHex'],
      }),
    ).toEqual({ ok: true, objectKind: 'shape', changed: true })

    const saved = await saveDocx(
      parsed,
      pmDocToSavePlan(editor.getJSON() as PmNode, parsed.blocks).saveBlocks,
    )
    const box = (await parseDocx(saved)).blocks.find((block) => block.textboxes)?.textboxes?.[0]
    expect(box?.fill).toBe('A1B2C3')
    expect(box?.borderColor).toBeUndefined()
    editor.destroy()
  })

  it('positions through the shared object action and preserves offsets after save/reopen', async () => {
    const { editor, parsed } = await openBlankDoc()
    insertShape(editor, 'ellipse')

    expect(
      setDocxObjectOffsetAtBlock(editor, {
        objectBlockIndex: 1,
        wrap: 'square-right',
        offsetXEmu: -95_250,
        offsetYEmu: 190_500,
    ...
    const reparsed = await parseDocx(saved)
    expect(reparsed.blocks.find((block) => block.textboxes)?.imageWrap).toBe('square-right')
    expect(reparsed.blocks.find((block) => block.textboxes)?.imageOffsetXEmu).toBe(-95_250)
    expect(reparsed.blocks.find((block) => block.textboxes)?.imageOffsetYEmu).toBe(190_500)
    editor.destroy()
  })

  it('resizes through the shared object action and preserves dimensions after save/reopen', async () => {
    const { editor, parsed } = await openBlankDoc()
    insertShape(editor, 'ellipse')

    expect(
      setDocxObjectSizeAtBlock(editor, {
        objectBlockIndex: 1,
        widthPx: 320,
        heightPx: 180,
      }),
    ...
    const reparsed = await parseDocx(saved)
    expect(reparsed.blocks.find((block) => block.textboxes)?.textboxes?.[0]).toMatchObject({
      prst: 'ellipse',
      widthPx: 320,
      heightPx: 180,
    })
  ...
  it('registry shape insertion survives save and reopen', async () => {
    const { editor, parsed } = await openBlankDoc()
    await expect(
    ...
    const reparsed = await parseDocx(saved)
    const shape = reparsed.blocks.find((block) => block.textboxes?.[0]?.prst === 'hexagon')
    expect(shape?.textboxes?.[0]).toMatchObject({
      widthPx: Math.round(WIDTH_EMU / 9_525),
      heightPx: Math.round(HEIGHT_EMU / 9_525),
    })
    editor.destroy()
  })

  it('registry arrow-line insertion survives save and reopen', async () => {
    const { editor, parsed } = await openBlankDoc()
    await expect(
      executeDocxOperation(
        editor,
    ...
    const reparsed = await parseDocx(saved)
    const line = reparsed.blocks.find((block) => block.textboxes?.[0]?.prst === 'lineArrowDouble')
    expect(line?.textboxes?.[0]).toMatchObject({
      readOnly: true,
      widthPx: Math.round(WIDTH_EMU / 9_525),
      heightPx: 12,
    })
