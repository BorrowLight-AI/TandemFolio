...
describe('explicit revision-tracking state', () => {
  it('sets the final recorder state and reports deterministic no-ops', () => {
    const editor = createEditor([para(text('base'))])
    expect(setDocxRevisionTracking(editor, { enabled: true })).toEqual({
      ok: true,
      enabled: true,
      changed: true,
    })
    expect((editor.storage.trackChanges as TrackChangesStorage).enabled).toBe(true)
    expect(setDocxRevisionTracking(editor, { enabled: true })).toEqual({
      ok: true,
      enabled: true,
      changed: false,
    })
    expect(setDocxRevisionTracking(editor, { enabled: false })).toEqual({
      ok: true,
      enabled: false,
      changed: true,
    })
  ...
  it('records and persists the next edit after explicit enablement', async () => {
    ...
    ;(editor.storage.trackChanges as TrackChangesStorage).author = 'Registry Reviewer'
    expect(setDocxRevisionTracking(editor, { enabled: true })).toMatchObject({ ok: true })
    editor.commands.insertContentAt(5, '!')
    expect(collectRevisions(editor.state.doc)).toMatchObject([
      { kind: 'ins', author: 'Registry Reviewer' },
    ])

    const plan = pmDocToSavePlan(editor.getJSON() as PmNode, parsed.blocks)
    const reopened = await parseDocx(await saveDocx(parsed, plan.saveBlocks))
    expect(reopened.internal.documentXml).toContain('<w:ins ')
    const reopenedEditor = new Editor({
      extensions: editorExtensions,
      content: blocksToPmDoc(reopened.blocks),
    })
    expect(collectRevisions(reopenedEditor.state.doc)).toMatchObject([
      { kind: 'ins', author: 'Registry Reviewer' },
    ])
    editor.destroy()
...
describe('explicit revision decisions', () => {
  it('accepts the current revision as one native Undo unit', () => {
    const editor = createEditor(revisedDoc())
    editor.commands.setTextSelection(5)
    expect(applyDocxRevisionDecision(editor, { decision: 'accept', scope: 'current' })).toEqual({
      ok: true,
      decision: 'accept',
      scope: 'current',
      matched: 1,
    ...
    expect(collectRevisions(editor.state.doc)).toHaveLength(2)
    editor.destroy()
  })

  it('rejects all revisions and saves/reopens the explicit final document', async () => {
    const parsed = await parseDocx(
      await buildDocx({
        bodyXml:
          '<w:p><w:r><w:t>A</w:t></w:r><w:ins w:id="1" w:author="Alice"><w:r><w:t>B</w:t></w:r></w:ins><w:del w:id="2" w:author="Bob"><w:r><w:delText>C</w:delText></w:r></w:del></w:p>',
      }),
    )
    const editor = new Editor({
      extensions: editorExtensions,
    ...
    expect(applyDocxRevisionDecision(editor, { decision: 'reject', scope: 'all' })).toEqual({
      ok: true,
      decision: 'reject',
      scope: 'all',
      matched: 2,
    ...
    const plan = pmDocToSavePlan(editor.getJSON() as PmNode, parsed.blocks)
    const reopened = await parseDocx(await saveDocx(parsed, plan.saveBlocks))
    expect(reopened.internal.documentXml).not.toMatch(/<w:(?:ins|del)\b/)
    const reopenedEditor = new Editor({
      extensions: editorExtensions,
      content: blocksToPmDoc(reopened.blocks),
    })
    expect(reopenedEditor.state.doc.textContent).toBe('AC')
    expect(collectRevisions(reopenedEditor.state.doc)).toHaveLength(0)
    editor.destroy()
    reopenedEditor.destroy()
  })
})
