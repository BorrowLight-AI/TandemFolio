...
} from '../src/renderer/editor/commands'
...
describe('updateParagraphStyle', () => {
  it('sets lineSpacing without touching align (fields mask)', () => {
    ...
    expect(centered.attrs.externalChanged).toBe(true)
...
describe('replaceAllText', () => {
  it('replaces every occurrence and keeps marks', () => {
    const editor = createEditor(fixtureDoc())
    const outcome = executeCommands(editor, {
      commands: [{ replaceAllText: { containsText: 'Acme', replaceText: 'Acme' } }],
    })
    expect(outcome.ok).toBe(true)
    expect(outcome.results[0].detail).toBe('replaced 2 occurrence(s)')
    const block = editor.state.doc.child(1)
    expect(block.textContent).toBe('Acme intro,Acme is great')
  ...
  it('matchCase: sensitive by default, insensitive when false', () => {
    ...
    const loose = executeCommands(editor, {
      commands: [{ replaceAllText: { containsText: 'acme', replaceText: 'X', matchCase: false } }],
    })
...
describe('transaction atomicity and externalChanged', () => {
  it('a multi-command round undoes in a single step', () => {
    ...
    const outcome = executeCommands(editor, {
      commands: [
        ...
        { replaceAllText: { containsText: 'Acme', replaceText: 'Acme' } },
  ...
  it('marks changed nodes externalChanged and leaves unmatched attrs untouched', () => {
    const editor = createEditor(fixtureDoc())
    ...
    expect(editor.state.doc.child(0).attrs.externalChanged).toBe(true)
    expect(editor.state.doc.child(2).attrs.externalChanged).toBe(true)
    expect({ ...editor.state.doc.child(3).attrs }).toEqual(untouchedBefore)
    expect(editor.state.doc.child(1).attrs.externalChanged).toBe(false)
  })
...
describe('createParagraphBullets / deleteParagraphBullets', () => {
  ...
  it('converts paragraphs to bullet list items with the context numId', () => {
    ...
    for (const i of [1, 3]) {
      ...
      expect(node.attrs.externalChanged).toBe(true)
    }
  })
