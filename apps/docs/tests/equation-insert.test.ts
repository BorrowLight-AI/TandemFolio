...
import { BUILTIN_EQUATIONS } from '../src/renderer/components/EquationModal'
import { blocksToPmDoc, pmDocToSavePlan, type PmNode } from '../src/renderer/editor/convert'
import {
  insertDocxEquation,
  insertEquationFromLatex,
  updateDocxEquation,
} from '../src/renderer/editor/equation-actions'
...
describe('equation insertion', () => {
  it('inserts through the shared action and preserves LaTeX-derived OMML after save/reopen', async () => {
    ...
    const formula = reparsed.blocks.find((block) => block.formulaDisplay)?.formulaDisplay
    expect(formula?.latex).toContain('\\frac')
    expect(formula?.latex).toContain('\\sqrt')
    expect(formula?.mathml).toContain('<mfrac>')
    expect(formula?.mathml).toContain('<msqrt>')
    editor.destroy()
  })
  ...
  it('persists both LaTeX rebuilds and structure-preserving token updates after reopen', async () => {
    const { editor, parsed } = await openDoc()
    insertEquationFromLatex(editor, 'a+b')
    expect(
      updateDocxEquation(editor, {
        placement: 'block',
        mode: 'latex',
        latex: '\\frac{x}{y}',
        tokens: null,
    ...
    const tokens = editor.state.doc.child(1).attrs.formulaDisplay.tokens as string[]
    expect(
      updateDocxEquation(editor, {
        placement: 'block',
        mode: 'tokens',
        latex: null,
        tokens: tokens.map((token, index) => (index === 0 ? 'n' : token)),
        equationBlockIndex: 1,
        from: null,
        to: null,
      }),
    ...
    const formula = reparsed.blocks.find((block) => block.formulaDisplay)?.formulaDisplay
    expect(formula?.tokens[0]).toBe('n')
    expect(formula?.mathml).toContain('<mfrac>')
    editor.destroy()
  })
