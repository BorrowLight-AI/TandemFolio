...
import { setDocxTextboxContentAtBlock } from '../src/renderer/editor/textbox-actions'
...
describe('text box rich-text edit and save', () => {
  ...
  it('sets bounded registry content and preserves rich paragraphs after save/reopen', async () => {
    const { editor, parsed } = await openTextboxDoc()
    expect(
      setDocxTextboxContentAtBlock(editor, {
        objectBlockIndex: 1,
        textboxIndex: 0,
        paragraphs: [
          {
            align: 'right',
            runs: [
              { text: 'Registry ', bold: true, color: '008000' },
              { text: 'content', italic: true, sizeHalfPoints: 30 },
            ],
          },
        ],
        heightPx: null,
      }),
    ).toMatchObject({ ok: true, changed: true, heightPx: expect.any(Number) })

    const plan = pmDocToSavePlan(editor.getJSON() as PmNode, parsed.blocks)
    const reparsed = await parseDocx(await saveDocx(parsed, plan.saveBlocks))
    expect(reparsed.blocks[1].textboxes?.[0].paras).toHaveLength(1)
    expect(reparsed.blocks[1].textboxes?.[0].paras[0]).toMatchObject({
      align: 'right',
      runs: [
        expect.objectContaining({ text: 'Registry ', bold: true, color: '008000' }),
        expect.objectContaining({ text: 'content', italic: true, sizeHalfPoints: 30 }),
      ],
    })
    editor.destroy()
  })
  ...
  it('sub-editor edits commit into the model on the pre-save broadcast', async () => {
    ...
    const restored = editor.state.doc.child(1).attrs.textboxes as TextboxDisplay[]
    expect(restored[0].paras[0].runs[0]).toMatchObject({
      color: '415461',
      font: 'Courier New',
    })
    editor.destroy()
