...
import { insertDocxTextboxAfterBlock } from '../src/renderer/editor/textbox-actions'
...
describe('text box insertion', () => {
  it('inserts through the shared action and preserves dimensions after save/reopen', async () => {
    const { editor, parsed } = await openBlankDoc()

    expect(
      insertDocxTextboxAfterBlock(editor, {
        afterBlockIndex: 0,
        widthEmu: 2_100_000,
        heightEmu: 1_200_000,
      }),
    ).toEqual({ ok: true, textboxBlockIndex: 1 })

    const plan = pmDocToSavePlan(editor.getJSON() as PmNode, parsed.blocks)
    const saved = await saveDocx(parsed, plan.saveBlocks)
    const reparsed = await parseDocx(saved)
    const textbox = reparsed.blocks.find((block) => block.textboxes)?.textboxes?.[0]
    expect(textbox).toMatchObject({
      fill: 'FFFFFF',
      borderColor: '000000',
      widthPx: Math.round(2_100_000 / 9_525),
      heightPx: Math.round(1_200_000 / 9_525),
    })
    editor.destroy()
  })
