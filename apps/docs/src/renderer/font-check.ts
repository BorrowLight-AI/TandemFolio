...
/** Visible text used to avoid loading script-specific fallback fonts the document never renders. */
export function collectDocText(parsed: ParsedDoc): string {
  const parts: string[] = []
  for (const block of parsed.blocks) {
    for (const run of block.runs ?? []) parts.push(run.text)
    if (block.previewText) parts.push(block.previewText)
    for (const textbox of block.textboxes ?? []) {
      for (const paragraph of textbox.paras) {
        for (const run of paragraph.runs) parts.push(run.text)
      }
    }
  }
  if (parsed.headerText) parts.push(parsed.headerText)
  if (parsed.footerText) parts.push(parsed.footerText)
  return parts.join('')
}
