...
import { docxImageOffsetPositionAttrs } from './image-actions'
import { updateDocxEquationAtPosition } from './equation-actions'
import { setDocxTextboxContentsAtPosition } from './textbox-actions'

/**
 * Custom schema mirroring the docx-engine Block model 1:1.
 * Every top-level node carries `docxIndex` (patch anchor, null = new) and
 * `externalChanged` (diff highlighting for external edits).
 */
...
import {
  removeSelectedDocxObject,
  setDocxObjectOffsetAtPosition,
  setDocxObjectSizeAtPosition,
} from './object-actions'
...
function blockAttrs(
  ...
  if (node.attrs.externalChanged) classes.push('external-changed')
...
/** Undo-owned full section settings attached to that section's last visible block. */
export const SectionSettingsOverrideExtension = Extension.create({
  name: 'sectionSettingsOverride',
  addGlobalAttributes() {
    return [
      {
        types: ['docParagraph', 'docHeading', 'docListItem', 'docTable', 'docProtected'],
        attributes: {
          sectionSettingsOverride: { default: null },
        },
      },
    ]
  },
...
/** Undo-owned full bibliography source snapshot attached to the first document block. */
export const SourceStateOverrideExtension = Extension.create({
  name: 'sourceStateOverride',
  addGlobalAttributes() {
    return [
      {
        types: ['docParagraph', 'docHeading', 'docListItem', 'docTable', 'docProtected'],
        attributes: {
          sourceStateOverride: { default: null },
        },
      },
    ]
  },
...
/** Undo-owned full comment metadata snapshot attached to the first document block. */
export const CommentStateOverrideExtension = Extension.create({
  name: 'commentStateOverride',
  addGlobalAttributes() {
    return [
      {
        types: ['docParagraph', 'docHeading', 'docListItem', 'docTable', 'docProtected'],
        attributes: {
          commentStateOverride: { default: null },
        },
      },
    ]
  },
...
export const DocNoteRef = Node.create({
  ...
  addAttributes() {
    return {
      ...
      id: { default: '' },
      num: { default: 1 },
      /** Body text for notes created in this editor; survives native Undo/Redo with the atom. */
      noteText: { default: null },
      /** True when note-part membership is governed by this reference's history state. */
...
export const DocInlineMath = Node.create({
  ...
  addAttributes() {
    return {
      ...
      /** flat token strip (word count / external read fallback) */
      text: { default: '' },
    }
  },
  parseHTML() {
      ...
      dom.addEventListener('dblclick', () => {
        ...
        if (latex && typeof pos === 'number') {
          window.dispatchEvent(
            new CustomEvent('docs-edit-inline-math', {
...
export const DocNestedTable = Node.create({
  ...
  addNodeView() {
    return ({ node, editor, getPos }) => {
      ...
      window.addEventListener('docs-commit-tables', commit)
      ...
      return {
        ...
        destroy: () => {
          window.removeEventListener('docs-commit-tables', commit)
...
export const DocProtected = Node.create({
  ...
  addAttributes() {
    return {
      ...
      /** New section's start type; kept on the inserted break node so native Undo removes it. */
      sectionStartType: { default: null as string | null },
...
/** Run object-mode deletion before the generic base keymap without reordering schema nodes. */
export const DocProtectedObjectRemovalShortcuts = Extension.create({
  name: 'docProtectedObjectRemovalShortcuts',
  priority: 1_000,
  addKeyboardShortcuts() {
    const removeSelected = () =>
      removeSelectedDocxObject(this.editor.state, this.editor.view.dispatch.bind(this.editor.view))
    return {
 ...
 * the table (or when App broadcasts 'docs-commit-tables' before saving).
 */
function wireTableEditing(
  ...
  return {
    setEditable,
    commit,
    cleanup: () => window.removeEventListener('docs-commit-tables', commit),
  }
}
...
function wireFormulaEditing(
  ...
  const commit = () => {
    ...
    const pos = getPos()
    if (typeof pos !== 'number') return
    updateDocxEquationAtPosition(view.state, view.dispatch.bind(view), pos, {
      placement: 'block',
      mode: 'tokens',
      latex: null,
      tokens,
...
function mountTextboxEditors(
  ...
  const commit = () => {
    ...
    const updates: Array<{
      textboxIndex: number
      paragraphs: TextboxPara[]
      heightPx: number | null
    }> = []
    model.forEach((box, i) => {
      const sub = editors[i]
      if (!sub || sub.isDestroyed) return
      ...
      if (same && sameHeight) return
      updates.push({ textboxIndex: i, paragraphs: paras, heightPx: measuredHeight ?? null })
    })
    if (updates.length === 0) return
    const pos = getPos()
    if (typeof pos !== 'number') return
    const result = setDocxTextboxContentsAtPosition(
      view.state,
    ...
    if (result.ok && result.changed) {
      knownBoxes = view.state.doc.nodeAt(pos)?.attrs.textboxes as TextboxDisplay[]
    }
  }
      ...
      handleDOMEvents: {
        mousedown: (view, event) => {
          ...
          if (target.classList?.contains('box-resize-handle')) {
            ...
            const maxW = isChart ? CHART_MAX_WIDTH_PX : 4096
            const maxH = 4096
            const sizeAt = (e: MouseEvent) => ({
              w: Math.min(maxW, Math.max(minW, startW + (e.clientX - startX) / zoom)),
              h: lockH
                ? startH
                : Math.min(maxH, Math.max(minH, startH + (e.clientY - startY) / zoom)),
            })
            ...
            const onUp = (e: MouseEvent) => {
              ...
              const { w, h } = sizeAt(e)
              const currentNode = view.state.doc.nodeAt(boxPos)
              const chart = currentNode?.attrs.chartDisplay as ChartDisplay | null | undefined
              setDocxObjectSizeAtPosition(view.state, view.dispatch.bind(view), boxPos, {
                widthPx: Math.round(w),
                heightPx: Math.round(h) + (chart?.title !== undefined ? CHART_TITLE_ROW_PX : 0),
              })
            }
      ...
      handleDOMEvents: {
        mousedown: (view, event) => {
          ...
          const onUp = (e: MouseEvent) => {
            ...
            if (isTextbox) {
              setDocxObjectOffsetAtPosition(view.state, view.dispatch.bind(view), pos, {
                wrap: currentNode.attrs.imageWrap ?? 'square-left',
                offsetXEmu: newX,
                offsetYEmu: newY,
              })
            } else {
              const previousWrap = (currentNode.attrs.imageWrap as string | null) ?? null
              const sideKind = previousWrap
                ? /^(square|tight|through)-(?:left|right)$/.exec(previousWrap)
                : null
              let nextWrap = previousWrap ?? 'square-left'
              if (!previousWrap || sideKind) {
                const kind = sideKind?.[1] ?? 'square'
                const centerX = startPxX + dx + imgW / 2
                nextWrap = `${kind}-${colW > 0 && centerX > colW / 2 ? 'right' : 'left'}`
              }
              const attrs = {
                ...currentNode.attrs,
                ...docxImageOffsetPositionAttrs(
                  nextWrap as ImageWrap,
                  Math.max(0, newX),
                  Math.max(0, newY),
                ),
              }
              if (
                ...
                const movedNode = currentNode.type.create(attrs, currentNode.content, currentNode.marks)
...
export const editorExtensions = [
  DocDocument,
  SectionSettingsOverrideExtension,
  SourceStateOverrideExtension,
  CommentStateOverrideExtension,
  DocText,
  DocHardBreak,
  DocNoteRef,
  DocXeMark,
  ...
  DocCellBoxes,
  DocProtected,
  DocProtectedObjectRemovalShortcuts,
  BoldMark,
  ItalicMark,
  UnderlineMark,
  StrikeMark,
