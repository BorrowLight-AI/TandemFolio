...
/** accept / reject only the revisions of one author (e.g. the external editor) */
...
export interface ApplyDocxRevisionDecisionInput {
  readonly decision: 'accept' | 'reject'
  readonly scope: 'current' | 'all'
}

export type ApplyDocxRevisionDecisionResult =
  | {
      readonly ok: true
      readonly decision: 'accept' | 'reject'
      readonly scope: 'current' | 'all'
...
/** Apply one bounded accept/reject decision through the mounted native-history route. */
export function applyDocxRevisionDecision(
  editor: Editor,
  input: ApplyDocxRevisionDecisionInput,
): ApplyDocxRevisionDecisionResult {
  if (
    (input?.decision !== 'accept' && input?.decision !== 'reject') ||
    (input?.scope !== 'current' && input?.scope !== 'all')
  ) {
    return {
      ok: false,
      error: 'invalid_arguments',
      message: 'DOCX revision decisions require accept/reject and current/all.',
    }
  }
  ...
  if (revisions.length === 0) {
    return {
      ...
      message: 'The mounted DOCX document has no revisions to handle.',
    }
  }
  const targets = input.scope === 'all' ? revisions : [currentRevision(editor)!]
  applyRevisions(editor, targets, input.decision)
  return {
    ok: true,
    decision: input.decision,
    scope: input.scope,
    matched: targets.length,
    remaining: collectRevisions(editor.state.doc).length,
    changed: true,
  }
}
...
export interface SetDocxRevisionTrackingInput {
  readonly enabled: boolean
}

export type SetDocxRevisionTrackingResult =
  | {
      readonly ok: true
      readonly enabled: boolean
...
/** Set the mounted recorder to one explicit final state without changing document history. */
export function setDocxRevisionTracking(
  editor: Editor,
  input: SetDocxRevisionTrackingInput,
): SetDocxRevisionTrackingResult {
  if (typeof input?.enabled !== 'boolean') {
    return {
      ok: false,
      error: 'invalid_arguments',
      message: 'DOCX revision tracking requires one explicit enabled boolean.',
    }
  }
  ...
  if (!storage) {
    return {
      ok: false,
      error: 'invalid_arguments',
      message: 'The mounted DOCX editor does not expose revision tracking state.',
    }
  }
  const changed = storage.enabled !== input.enabled
  storage.enabled = input.enabled
  return { ok: true, enabled: input.enabled, changed }
}
