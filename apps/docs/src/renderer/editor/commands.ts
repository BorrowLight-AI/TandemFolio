...
import { dirFlipAttrs } from './direction'
import { normalizeListLevel } from './indent'
import { isTrackedDeleted, liveText } from './live-text'
import { serializeParagraphTabStops, validateParagraphTabStops } from './tab-stops'

/**
 * Structured edit commands: a caller supplies a batchUpdate-style JSON envelope
 * and code applies it deterministically on the
...
export interface UpdateParagraphStyle {
  ...
  style: {
    ...
    lineRule?: 'auto' | 'atLeast' | 'exact' | null
...
export interface SetParagraphDirection {
  target: Target
  direction: 'ltr' | 'rtl'
}

export interface SetListLevel {
  target: Target
  level: number
}

export interface ApplyListPreset {
  target: Target
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    switch (name as CommandName) {
      ...
      case 'updateParagraphStyle': {
        ...
        if (!error) {
          const style = payload.style as UpdateParagraphStyle['style']
          const boundedNumber = (value: unknown, minimum: number, maximum: number): boolean =>
            value === undefined ||
            value === null ||
            (typeof value === 'number' &&
              ...
              value >= minimum &&
              value <= maximum)
          if (!boundedNumber(style.lineSpacing, 0.06, 132)) {
            error = `${where}: lineSpacing must be null or between 0.06 and 132`
          } else if (
            style.lineRule !== undefined &&
            style.lineRule !== null &&
            !['auto', 'atLeast', 'exact'].includes(style.lineRule)
          ) {
            error = `${where}: lineRule must be auto, atLeast, exact, or null`
          } else if (
            style.lineRawTwips !== undefined &&
            style.lineRawTwips !== null &&
            (!Number.isInteger(style.lineRawTwips) ||
              style.lineRawTwips < 20 ||
              style.lineRawTwips > 31680)
          ) {
            error = `${where}: lineRawTwips must be null or an integer between 20 and 31680`
          } else if (
            !boundedNumber(style.indentLeft, -31680, 31680) ||
            !boundedNumber(style.indentRight, -31680, 31680) ||
            !boundedNumber(style.indentFirstLine, -31680, 31680)
          ) {
            error = `${where}: paragraph indents must be null or between -31680 and 31680`
          } else if (
            !boundedNumber(style.spaceBefore, 0, 31680) ||
            !boundedNumber(style.spaceAfter, 0, 31680)
          ) {
            error = `${where}: paragraph spacing must be null or between 0 and 31680`
          } else {
            const tabStopsError = validateParagraphTabStops(style.tabStops)
            if (tabStopsError) error = `${where}: ${tabStopsError}`
          }
        }
        break
      }
      case 'setParagraphDirection':
        ...
        if (!error && payload.direction !== 'ltr' && payload.direction !== 'rtl') {
          error = `${where}: direction must be ltr or rtl`
        }
        break
      case 'setListLevel':
        error = validateTarget(payload.target, where)
        if (
          ...
          (!Number.isInteger(payload.level) ||
            ...
            Number(payload.level) > 8)
        ) {
          error = `${where}: level must be an integer between 0 and 8`
        }
        break
      case 'applyListPreset': {
        error = validateTarget(payload.target, where)
        const levels = payload.levels as CustomNumberingLevel[] | undefined
        if (!error && (!Array.isArray(levels) || levels.length < 1 || levels.length > 9)) {
          error = `${where}: levels must contain 1 through 9 items`
        }
        if (!error && levels) {
          for (let level = 0; level < levels.length; level++) {
            const item = levels[level]
            if (!(LIST_NUM_FORMATS as readonly string[]).includes(item.numFmt)) {
              error = `${where}: level ${level} has an unsupported numFmt`
              break
            }
            if (
              typeof item.lvlText !== 'string' ||
              item.lvlText.length < 1 ||
              item.lvlText.length > 64 ||
              /[\u0000-\u001f\u007f]/.test(item.lvlText)
            ) {
              error = `${where}: level ${level} lvlText must contain 1 through 64 visible characters`
              break
            }
            if (
              !Number.isInteger(item.indentLeft) ||
              item.indentLeft < 0 ||
              item.indentLeft > 31680 ||
              (item.hanging !== undefined &&
                (!Number.isInteger(item.hanging) || item.hanging < 0 || item.hanging > 31680)) ||
              (item.start !== undefined &&
                (!Number.isInteger(item.start) || item.start < 1 || item.start > 1_000_000))
            ) {
              error = `${where}: level ${level} has out-of-range numbering geometry`
              break
            }
          }
        }
...
function markChanged(tr: Transaction, pos: number): boolean {
  ...
  tr.setNodeMarkup(pos, undefined, { ...node.attrs, externalChanged: true })
  return true
}
...
function runUpdateParagraphStyle(
  ...
  for (const b of matched) {
    ...
    for (const f of cmd.fields) {
      attrs[f] =
        f === 'tabStops'
          ? serializeParagraphTabStops(cmd.style.tabStops ?? null)
          : (cmd.style[f] ?? (f === 'pageBreakBefore' ? false : null))
    }
    const dirty = cmd.fields.some((f) => attrs[f] !== b.node.attrs[f])
    if (!dirty) continue
    tr.setNodeMarkup(b.pos, undefined, { ...attrs, externalChanged: true })
    changed++
  }
  return { command: 'updateParagraphStyle', matched: matched.length, changed, skippedProtected }
}

function runSetParagraphDirection(
  tr: Transaction,
  cmd: SetParagraphDirection,
  sel: SelRange,
): CommandResult {
  const matched = matchTarget(tr.doc, cmd.target, sel)
  const bidi = cmd.direction === 'rtl'
  let changed = 0
  let skippedProtected = 0
  for (const block of matched) {
    if (block.node.type.name === 'docProtected') {
      skippedProtected++
      continue
    }
    if (block.node.attrs.bidi === undefined || block.node.attrs.bidi === bidi) continue
    tr.setNodeMarkup(block.pos, undefined, {
      ...dirFlipAttrs(block.node.attrs, bidi),
...
function runSetListLevel(tr: Transaction, cmd: SetListLevel, sel: SelRange): CommandResult {
  const matched = matchTarget(tr.doc, cmd.target, sel)
  const level = normalizeListLevel(cmd.level)
  let changed = 0
  let skippedProtected = 0
  for (const block of matched) {
    if (block.node.type.name === 'docProtected') {
    ...
    if (block.node.type.name !== 'docListItem' || block.node.attrs.ilvl === level) continue
    tr.setNodeMarkup(block.pos, undefined, {
      ...block.node.attrs,
      ilvl: level,
      externalChanged: true,
  ...
  return { command: 'setListLevel', matched: matched.length, changed, skippedProtected }
}

function runApplyListPreset(
  tr: Transaction,
  schema: Schema,
  cmd: ApplyListPreset,
  sel: SelRange,
  ...
  const matched = matchTarget(tr.doc, cmd.target, sel)
  const editable = matched.filter(
    (block) => block.node.type.name === 'docParagraph' || block.node.type.name === 'docListItem',
  )
  const skippedProtected = matched.filter((block) => block.node.type.name === 'docProtected').length
  if (editable.length === 0) {
    return { command: 'applyListPreset', matched: matched.length, changed: 0, skippedProtected }
  }
  const numId = context.createListDef?.(cmd.levels)
  if (!numId) throw new Error('applyListPreset: could not create a numbering definition')
  const kind = cmd.levels[0].numFmt === 'bullet' ? 'bullet' : 'ordered'
  for (const block of editable) {
    const ilvl =
      block.node.type.name === 'docListItem'
        ? Math.min(normalizeListLevel(Number(block.node.attrs.ilvl) || 0), cmd.levels.length - 1)
        : 0
    tr.setNodeMarkup(block.pos, schema.nodes.docListItem, {
      ...block.node.attrs,
      styleId: null,
      kind,
      numId,
  ...
  return {
    command: 'applyListPreset',
    matched: matched.length,
    changed: editable.length,
    skippedProtected,
    detail: String(cmd.levels.length),
  }
}
...
function runReplaceAllText(tr: Transaction, schema: Schema, cmd: ReplaceAllText): CommandResult {
  ...
  return {
    ...
    skippedProtected,
    skippedDeleted,
    detail: `replaced ${replacements.length} occurrence(s)`,
  }
}
...
function runMoveBlocks(tr: Transaction, cmd: MoveBlocks): CommandResult {
  ...
  // moved blocks keep document order; externalChanged marks the landing highlight
  const moved = [...movedSet]
    .sort((a, z) => a - z)
    .map((i) => {
      const node = blocks[i].node
      if (node.type.name === 'docProtected') return node
      return node.type.create({ ...node.attrs, externalChanged: true }, node.content, node.marks)
    })
...
function runCreateParagraphBullets(
  ...
  let numId = context.numIds?.[kind] ?? null
  let numIdResolved = false
  const resolveNumId = (): string | null => {
    if (!numIdResolved) {
      numIdResolved = true
      if (numId === null && context.allocateNumId) numId = context.allocateNumId(kind)
    }
    if (numId === null && context.allocateNumId) {
      throw new Error('createParagraphBullets: could not allocate a numbering definition')
    }
    return numId
  }
  let changed = 0
  ...
  for (const b of matched) {
    ...
    if (b.node.type.name === 'docListItem') {
      ...
      const resolvedNumId = resolveNumId()
      tr.setNodeMarkup(b.pos, undefined, {
        ...b.node.attrs,
        kind,
        numId: resolvedNumId ?? b.node.attrs.numId,
        externalChanged: true,
      })
      changed++
      continue
...
function summarize(results: CommandResult[]): string {
  ...
  if (total === 0) {
    ...
    return skipped > 0
      ? `No editable blocks matched; skipped ${skipped} protected block(s).`
      : 'No blocks matched; the document was not changed.'
  }
  const parts = results.map((r) => {
    let part: string
    switch (r.command) {
      case 'updateTextStyle':
        part = `Updated text style in ${r.changed} block(s)`
        break
      case 'updateParagraphStyle':
        part = `Updated paragraph style in ${r.changed} block(s)`
        break
      case 'setParagraphDirection':
        part = `Updated paragraph direction in ${r.changed} block(s)`
        break
      case 'setListLevel':
        part = `Updated list level in ${r.changed} block(s)`
        break
      case 'applyListPreset':
        part = `Applied a ${r.detail}-level list preset to ${r.changed} block(s)`
        break
      case 'setHeadingLevel':
        part = `Updated heading level in ${r.changed} block(s)`
        break
      case 'replaceAllText':
        part = `${r.detail ?? 'Replaced text'} in ${r.changed} block(s)`
        break
      case 'deleteBlocks':
        part = `Deleted ${r.changed} block(s)`
        break
      case 'moveBlocks':
        part = `Moved ${r.changed} block(s)`
        break
      case 'createParagraphBullets':
        part = `Converted ${r.changed} block(s) to list items`
        break
      case 'deleteParagraphBullets':
        part = `Converted ${r.changed} list item(s) to body text`
        break
      case 'updateImageProperties':
        part = `Updated ${r.changed} image(s)`
        break
      case 'insertToc':
        part = `Inserted a table of contents with ${r.matched} entries`
        break
      default:
        part = `${r.command}: ${r.changed}`
    }
    if (r.skippedProtected > 0) part += `; skipped ${r.skippedProtected} protected block(s)`
    return part
  })
  return parts.join(';')
}
...
export function executeCommands(
  ...
  try {
    for (const command of envelope.commands) {
      ...
      } else if ('setParagraphDirection' in command) {
        results.push(runSetParagraphDirection(tr, command.setParagraphDirection, sel))
      } else if ('setListLevel' in command) {
        results.push(runSetListLevel(tr, command.setListLevel, sel))
      } else if ('applyListPreset' in command) {
        results.push(runApplyListPreset(tr, schema, command.applyListPreset, sel, context))
  ...
  if (tr.steps.length > 0) {
    // Route through the TrackChanges recorder when the caller requests a named author.
