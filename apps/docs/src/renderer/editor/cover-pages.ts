...
export const DOCX_COVER_PRESET_IDS = [
  'classic',
  'banded',
  'boxed',
  'sideline',
...
export type DocxCoverPresetId = (typeof DOCX_COVER_PRESET_IDS)[number]

export interface InsertDocxCoverPageInput {
  readonly preset: DocxCoverPresetId
...
export type InsertDocxCoverPageResult =
  | {
      readonly ok: true
      readonly preset: DocxCoverPresetId
...
export function defaultCoverPageContent(): Omit<InsertDocxCoverPageInput, 'preset'> {
  return {
    title: TITLE(),
    subtitle: SUBTITLE(),
    author: AUTHOR(),
...
export function buildCoverNodes(
  preset: CoverPreset,
  content: Omit<InsertDocxCoverPageInput, 'preset'> = defaultCoverPageContent(),
): PmNode[] {
  const defaults = defaultCoverPageContent()
  const substitute = (text: string | undefined): string | undefined => {
    if (text === undefined) return undefined
    if (text === String(defaults.year)) return String(content.year)
    return text
      .replaceAll(defaults.date, content.date)
      .replaceAll(defaults.title, content.title)
      .replaceAll(defaults.subtitle, content.subtitle)
      .replaceAll(defaults.author, content.author)
  }
  return [
    ...preset.paras.map((paragraph) => paraNode({ ...paragraph, text: substitute(paragraph.text) })),
    {
      type: 'docParagraph',
      attrs: { docxIndex: null, externalChanged: false, pageBreakBefore: true },
    },
  ]
}
...
export function insertCoverPage(editor: Editor, preset: CoverPreset): void {
  void insertDocxCoverPage(editor, { preset: preset.id as DocxCoverPresetId, ...defaultCoverPageContent() })
}

function boundedText(value: unknown): value is string {
  return typeof value === 'string' && Array.from(value).length >= 1 && Array.from(value).length <= 255
}

/** Insert one deterministic built-in cover as a single native history transaction. */
export function insertDocxCoverPage(
  editor: Editor,
  input: InsertDocxCoverPageInput,
): InsertDocxCoverPageResult {
  const preset = COVER_PRESETS.find((candidate) => candidate.id === input?.preset)
  if (
    !preset ||
    !boundedText(input?.title) ||
    !boundedText(input?.subtitle) ||
    !boundedText(input?.author) ||
    !boundedText(input?.date) ||
    ...
    return {
      ...
      message: 'DOCX cover-page insertion requires a known preset and bounded explicit content.',
    }
  }
  const nodes = buildCoverNodes(preset, input)
  if (!editor.chain().focus().insertContentAt(0, nodes).run()) {
    return {
      ok: false,
      error: 'execution_failed',
      message: 'The DOCX cover page could not be inserted at the document start.',
    }
  }
  return { ok: true, preset: input.preset, insertedBlocks: nodes.length, changed: true }
}
