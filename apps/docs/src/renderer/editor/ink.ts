...
export interface DocxInkStrokeInput extends InkStroke {
  readonly id: string
  readonly anchorIndex: number
}

export type ApplyDocxInkInput =
  | { readonly action: 'add'; readonly annotation: DocxInkStrokeInput }
  | { readonly action: 'delete'; readonly ids: readonly string[] }
  | { readonly action: 'clear' }

export type ApplyDocxInkResult =
  | {
      readonly ok: true
      readonly action: 'add' | 'delete' | 'clear'
      readonly added: number
      readonly deleted: number
      readonly count: number
      readonly changed: boolean
      readonly annotations: readonly InkAnnotation[]
    }
  | {
      readonly ok: false
...
const validInkId = (value: unknown): value is string =>
  typeof value === 'string' &&
  Array.from(value).length >= 1 &&
  Array.from(value).length <= 128 &&
  /^[A-Za-z0-9_.:-]+$/.test(value)

/** Apply one bounded add/delete/clear action to the renderer-owned ink layer. */
export function applyDocxInk(
  current: readonly InkAnnotation[],
  input: ApplyDocxInkInput,
): ApplyDocxInkResult {
  if (input?.action === 'add') {
    const annotation = input.annotation
    const validPoint = (point: InkPoint) =>
    ...
    if (
      !annotation ||
      !validInkId(annotation.id) ||
      current.some((entry) => entry.id === annotation.id) ||
      !Number.isInteger(annotation.anchorIndex) ||
      annotation.anchorIndex < 0 ||
      annotation.anchorIndex > 2_147_483_647 ||
      (annotation.tool !== 'pen' && annotation.tool !== 'highlighter') ||
      !/^[0-9A-Fa-f]{6}$/.test(annotation.color) ||
      ...
      annotation.points.length > 4096 ||
      !annotation.points.every(validPoint)
    ) {
      return {
        ok: false,
        error: 'invalid_arguments',
        message: 'DOCX ink addition requires one unique bounded vector annotation.',
      }
    }
    ...
    const annotations = [...current, next]
    return {
      ok: true,
      action: 'add',
      added: 1,
  ...
  if (input?.action === 'delete') {
    ...
    if (!Array.isArray(ids)) {
      return {
        ok: false,
        error: 'invalid_arguments',
        message: 'DOCX ink deletion requires 1–1024 unique existing stable IDs.',
      }
    }
    ...
    const annotations = current.filter((entry) => !unique.has(entry.id))
    return {
      ok: true,
      action: 'delete',
      added: 0,
  ...
  return {
    ok: false,
    error: 'invalid_arguments',
    message: 'DOCX ink requires add, delete, or clear.',
  }
}
