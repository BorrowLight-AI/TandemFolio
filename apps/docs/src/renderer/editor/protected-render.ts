...
import { updateDocxChartAtPosition, type DocxChartUpdateField } from './chart-actions'
...
export function wireChartEditing(
  ...
  const commit = () => {
    ...
    const patch: {
      title?: string
      categories?: string[]
      series?: Array<{ name: string | null; values: (number | null)[] }>
    } = {}
    const fields: DocxChartUpdateField[] = []
    if (next.title !== undefined && next.title !== model.title) {
      patch.title = next.title
      fields.push('title')
    }
    if (JSON.stringify(next.categories) !== JSON.stringify(model.categories)) {
      patch.categories = next.categories
      fields.push('categories')
    }
    if (JSON.stringify(next.series) !== JSON.stringify(model.series)) {
      patch.series = next.series.map((series) => ({
    ...
    updateDocxChartAtPosition(view.state, view.dispatch.bind(view), pos, { patch, fields })
  }
