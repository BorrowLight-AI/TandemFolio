...
function makeRevBubble(kind: 'del' | 'fmt', text: string): HTMLElement {
  ...
  label.textContent = `${t((kind === 'del' ? 'editorRevDeleted' : 'editorRevFormatted') as any)}: `
...
function fmtDesc(old: Record<string, unknown>, marks: readonly PmMark[]): string {
  ...
  if (fontBits.length > 0) parts.push(`${t('editorRevFont' as any)}: ${fontBits.join(', ')}`)
  if (bold !== !!old.bold) parts.push(t((bold ? 'editorRevBold' : 'editorRevNotBold') as any))
  if (italic !== !!old.italic) parts.push(t((italic ? 'editorRevItalic' : 'editorRevNotItalic') as any))
  ...
  if (newHl !== (((old.highlight as string | undefined) ?? null) as string | null)) {
    parts.push(t('editorRevHighlight' as any))
  }
