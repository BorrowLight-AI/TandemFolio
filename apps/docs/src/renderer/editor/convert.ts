...
export function blocksToPmDoc(blocks: Block[], sections?: SectionInfo[]): PmNode {
  ...
  if (content.length === 0) {
    content.push({
      type: 'docParagraph',
      attrs: { docxIndex: null, styleId: null, externalChanged: false },
    })
  }
...
export interface SavePlan {
  ...
  /** New break-node locations whose start type must be projected onto the following sectPr. */
  sectionBreakStarts: Array<{
    saveBlockIndex: number
    startType: 'nextPage' | 'continuous' | 'evenPage' | 'oddPage'
  }>
  /** Undo-owned full settings attached to section anchor blocks. */
  sectionSettingsOverrides: Array<{
    sectionIndex: number
    settings: SectionSettings
    titlePg?: boolean
    evenOddHeaders?: boolean
    pageNumbering?: {
      format:
        | 'decimal'
        | 'numberInDash'
...
export function pmDocToSavePlan(doc: PmNode, originalBlocks: Block[]): SavePlan {
  ...
  const sectionBreakStarts: SavePlan['sectionBreakStarts'] = []
  const sectionSettingsOverrides: SavePlan['sectionSettingsOverrides'] = []
  ...
  for (const node of doc.content ?? []) {
    const sectionSettingsOverride = node.attrs?.sectionSettingsOverride as
      SavePlan['sectionSettingsOverrides'][number] | null | undefined
    if (
      sectionSettingsOverride &&
      Number.isInteger(sectionSettingsOverride.sectionIndex) &&
      sectionSettingsOverride.sectionIndex >= 0 &&
      sectionSettingsOverride.settings
    ) {
      sectionSettingsOverrides.push(sectionSettingsOverride)
    }
    ...
    if (node.type === 'docProtected') {
      ...
      if (idx !== null && idx !== undefined && originalByIndex.has(idx) && !usedIndexes.has(idx)) {
        ...
        const saveBlockIndex = saveBlocks.length
        pushBlock({ kind: 'xml', xml })
        const sectionStartType = node.attrs?.sectionStartType
        if (
          ...
          sectionBreakStarts.push({ saveBlockIndex, startType: sectionStartType })
        }
...
function imagePatchOf(node: PmNode, original: Block): ImageBlockPatch | null {
  ...
  const crop = (node.attrs?.imageCrop as NewImage['crop'] | null) ?? null
  const originalCrop = original.imageCrop ?? null
  if (
    crop?.l !== originalCrop?.l ||
    crop?.t !== originalCrop?.t ||
    crop?.r !== originalCrop?.r ||
    crop?.b !== originalCrop?.b
  ) {
    patch.crop = crop
  }
