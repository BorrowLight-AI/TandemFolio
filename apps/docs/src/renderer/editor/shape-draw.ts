...
import { setDocxObjectOffsetAtPosition } from './object-actions'
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        ...
        setDocxObjectOffsetAtPosition(
          ...
          insertedAt,
          {
            wrap: node.attrs.imageWrap ?? 'square-left',
            offsetXEmu: Number(node.attrs.imageOffsetXEmu ?? 0) + Math.round(dx * EMU_PER_PX),
            offsetYEmu: Number(node.attrs.imageOffsetYEmu ?? 0) + Math.round(dy * EMU_PER_PX),
          },
          { closeHistory: false },
        )
      }),
