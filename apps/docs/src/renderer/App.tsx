...
import { serializeParagraphTabStops } from './editor/tab-stops'
import {
  insertDocxField,
  type DocxFieldInstruction,
  updateDocxFields,
...
import { collectDocxSources, upsertDocxSource } from './editor/source-actions'
import { collectDocxComments } from './editor/comments'
...
import { McpFullscreenButton } from './host/McpFullscreenButton'
import { createDocxRecoverySnapshot, loadStagedDocx } from './host/docx-commands'
import { buildDocxSelectionContext } from './host/document-context'
import { executeDocxOperation } from './operations/registry'
import { insertSectionBreakAfterBlock, resolveSectionBreakSource } from './editor/section-actions'
import {
  applySectionSettingsOverrides,
  collectSectionSettingsOverrides,
  isDocxPageNumberFormat,
  setDocumentDifferentOddEvenPages,
  setHeaderFooterValue,
  setHeaderFooterPageNumber,
  setSectionDifferentFirstPage,
  setSectionLayoutOverride,
  setSectionPageNumbering,
  type DocxPageNumberFormat,
...
import {
  ...
  continueNumberingAt as continueNumberingAtImpl,
  continueNumbering as continueNumberingImpl,
  createCustomListDef as createCustomListDefImpl,
  restartNumberingAt as restartNumberingAtImpl,
...
function commitDocumentDesignState(
  state: FileActionContext,
  design: DocxDocumentDesignState,
  fields: readonly DocxDocumentDesignField[],
): void {
  if (fields.includes('pageColor')) {
    state.setPageColor(design.pageColor)
    state.setPageColorDirty(true)
  }
  if (fields.includes('watermark')) {
    state.setWatermark(design.watermark)
    state.setWatermarkDirty(true)
  }
  if (fields.includes('themeFonts')) {
    state.setThemeFonts(design.themeFonts)
    state.setThemeFontsDirty(true)
  }
  if (fields.includes('themeColors')) {
    state.setThemeColors(design.themeColors)
    state.setThemeColorsDirty(true)
  }
  state.dirtyRef.current = true
}
...
function subscribeDocxEditorActivity(listener: () => void): () => void {
  return subscribeLiveEditorActivity(() => {
    if (!getLiveEditorActivity()) {
      // Protected tables and textbox sub-editors intentionally keep pending edits
      // in their own DOM until focus leaves. Flush them into the authoritative
      // ProseMirror document before React releases those node views.
      window.dispatchEvent(new Event('docs-commit-tables'))
    }
    listener()
  })
}
...
export function App() {
  ...
  /** Page-number-format dialog; accepted state is written through the Undo-owned section journal. */
  const [pgNumModal, setPgNumModal] = useState<{
    fmt: DocxPageNumberFormat
    start: string
  } | null>(null)
  ...
  /** Render mirror of the Undo-owned header/footer journal; never used as save authority. */
  const [headerFooterOverrides, setHeaderFooterOverrides] = useState<
    Array<HeaderFooterEdit & { sectionIndex: number }>
  >([])
  ...
  /** Effective indexed part: Undo journal override first, then the parsed section reference. */
  const headerFooterValueAt = useCallback(
    (
      sectionIndex: number,
      kind: 'header' | 'footer',
      variant: HeaderFooterVariant,
    ): HeaderFooter | null => {
      const edited = headerFooterOverrides.find(
        (entry) =>
          entry.sectionIndex === sectionIndex && entry.kind === kind && entry.variant === variant,
      )
      if (edited) return edited.value
      const rId = effRefsAll[sectionIndex]?.[kind]?.[variant]
      if (rId) return hfFromPart(doc?.parsed.hfParts?.[rId])
      if (sectionIndex !== sections.length - 1) return null
      if (variant === 'default') return kind === 'header' ? header : footer
      const key: HfVariantKey = `${kind}${variant === 'first' ? 'First' : 'Even'}`
      return hfVariants[key]
    },
    [headerFooterOverrides, effRefsAll, doc, sections.length, header, footer, hfVariants],
  )
  ...
  const shownHeader = headerFooterValueAt(hfSectionClamped, 'header', headerAreaView)
  const shownFooter = headerFooterValueAt(hfSectionClamped, 'footer', footerAreaView)
  ...
  const commitHfVariant = (
    ...
    setHeaderFooterValue(
      editor,
      { sectionIndex: hfSectionClamped, kind, variant, value: next },
      sections,
      headerFooterValueAt(hfSectionClamped, kind, variant),
    )
  }
  const commitHf = (kind: 'header' | 'footer', next: HeaderFooter) =>
    commitHfVariant(kind, kind === 'header' ? headerAreaView : footerAreaView, next)
  ...
  const toggleTitlePg = (on: boolean) => {
    ...
    const result = setSectionDifferentFirstPage(
      editor,
      { sectionIndex: hfSectionClamped, enabled: on },
      sections,
    )
    if (!result.ok) return
    setTitlePg(on)
    setSections((current) =>
      current.map((section, index) =>
        index === hfSectionClamped ? { ...section, titlePg: on } : section,
      ),
    )
  ...
  const sectionHfOverride = useCallback(
    (si: number, kind: 'header' | 'footer'): HeaderFooter | null => {
      return (
        headerFooterOverrides.find(
          (entry) =>
            entry.sectionIndex === si && entry.kind === kind && entry.variant === 'default',
        )?.value ?? null
      )
    },
  ...
  const commentStateRef = useRef<CommentInfo[]>(comments)
  const parsedCommentsRef = useRef<readonly CommentInfo[]>(doc?.parsed.comments ?? [])
  commentStateRef.current = comments
  parsedCommentsRef.current = doc?.parsed.comments ?? []
  ...
  noteStateRef.current = { footnotes, endnotes }
  const [sources, setSources] = useState<SourceInfo[]>([])
  const [sourcesDirty, setSourcesDirty] = useState(false)
  const sourceStateRef = useRef<SourceInfo[]>(sources)
  const parsedSourcesRef = useRef<readonly SourceInfo[]>(doc?.parsed.sources ?? [])
  sourceStateRef.current = sources
  parsedSourcesRef.current = doc?.parsed.sources ?? []
  ...
  const [autoSave, setAutoSave] = useState(
    () => localStorage.getItem('tandemfolio.autoSave') === '1',
  )
  ...
  const mcpRecoveryVersionRef = useRef(0)
  const mcpRecoveryStateRef = useRef<readonly unknown[] | null>(null)
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: updatedEditor }) => {
      ...
      if (managedNotesRef.current.size > 0) {
        const current = noteStateRef.current
        const next = collectDocxNotes(
          updatedEditor,
          current.footnotes,
          current.endnotes,
        ...
        if (JSON.stringify(next) !== JSON.stringify(current)) {
          ...
          setEndnotes(next.endnotes)
          setNotesDirty(true)
        }
      }
      const nextSources = collectDocxSources(updatedEditor, parsedSourcesRef.current)
      if (JSON.stringify(nextSources) !== JSON.stringify(sourceStateRef.current)) {
        sourceStateRef.current = nextSources
        setSources(nextSources)
        setSourcesDirty(true)
      }
      const nextComments = collectDocxComments(updatedEditor, parsedCommentsRef.current)
      if (JSON.stringify(nextComments) !== JSON.stringify(commentStateRef.current)) {
        commentStateRef.current = nextComments
  ...
  useEffect(
    () =>
      subscribeSubEditorState((docChanged) => {
        if (docChanged) {
          dirtyRef.current = true
          mcpRecoveryVersionRef.current += 1
        }
        forceRender()
      }),
    [],
  ...
  useEffect(() => {
    const notePendingProtectedEdit = (event: Event) => {
      const target = event.target as HTMLElement | null
      if (!target?.closest('.doc-protected, .doc-nested-table')) return
      dirtyRef.current = true
      mcpRecoveryVersionRef.current += 1
    }
    window.addEventListener('input', notePendingProtectedEdit, true)
    return () => window.removeEventListener('input', notePendingProtectedEdit, true)
  }, [])
  ...
  useEffect(() => {
    localStorage.setItem('tandemfolio.autoSave', autoSave ? '1' : '0')
  }, [autoSave])
  ...
  // Keep host chrome in sync with the renderer's appearance.
  useEffect(() => {
  ...
  useEffect(() => {
    if (!editor) return
    let signature = JSON.stringify(collectSectionSettingsOverrides(editor))
    const syncSectionOverrides = () => {
      const overrides = collectSectionSettingsOverrides(editor)
      const nextSignature = JSON.stringify(overrides)
      if (nextSignature === signature) return
      signature = nextSignature
      setHeaderFooterOverrides(
        overrides.flatMap((override) =>
          (override.headerFooterEdits ?? []).map((edit) => ({
            sectionIndex: override.sectionIndex,
            ...edit,
          })),
        ),
      )
      const parsed = fileCtxRef.current.doc?.parsed
      if (!parsed) return
      const effective = applySectionSettingsOverrides(readSections(parsed), overrides)
      setSections((current) =>
        current.map((section, index) => ({
          ...section,
          settings: effective[index]?.settings ?? section.settings,
          titlePg: effective[index]?.titlePg ?? section.titlePg,
          pageNumberFmt: effective[index]?.pageNumberFmt,
          pageNumberStart: effective[index]?.pageNumberStart,
        })),
      )
      if (effective[0]) setTitlePg(effective[0].titlePg)
      const oddEvenOverride = overrides.find((override) => override.evenOddHeaders !== undefined)
      setEvenOddHf(oddEvenOverride?.evenOddHeaders ?? parsed.evenAndOddHeaders ?? false)
      const finalSettings = effective[effective.length - 1]?.settings
      if (finalSettings) setSection(finalSettings)
    }
    editor.on('update', syncSectionOverrides)
    return () => {
      editor.off('update', syncSectionOverrides)
    }
  }, [editor])
  ...
  useEffect(() => {
    ...
    // StrictMode runs the mount effect twice but the pending queues can only be consumed
    // once, so the consume Promise lives in a ref and its result is processed only once.
  ...
  const save = useCallback(async (saveAs: boolean, auto = false) => {
    const saved = await saveImpl(fileCtxRef.current, saveAs, auto)
    if (saved) managedNotesRef.current.clear()
    return saved
  }, [])

  // Inserting a section break needs one save/reparse for section geometry to take effect.
  // The break's start type itself lives on the inserted ProseMirror node, so Undo removes it.
  ...
  const insertSectionBreak = useCallback(
    (type: SectionInfo['startType']) => {
      ...
      const result = insertSectionBreakAfterBlock(
        editor,
        { afterBlockIndex, startType: type },
        (boundary) => resolveSectionBreakSource(editor, boundary, sections, section),
      )
      if (!result.ok) return
  ...
  const restartList = useCallback(
    (input: Parameters<typeof restartNumberingAtImpl>[1]) =>
      restartNumberingAtImpl(numberingCtxRef.current, input),
    [],
  )
  const continueNumbering = useCallback(() => continueNumberingImpl(numberingCtxRef.current), [])
  const continueList = useCallback(
    (input: Parameters<typeof continueNumberingAtImpl>[1]) =>
      continueNumberingAtImpl(numberingCtxRef.current, input),
    [],
  )
  ...
  const insertField = useCallback(
    (instr: string) => {
      ...
      const result = insertDocxField(editor, {
        range: { from, to },
        instruction: instr as DocxFieldInstruction,
        displayText: value,
      })
      if (!result.ok) {
  ...
  const updateFields = useCallback(() => {
    if (!editor) return
    const { state } = editor
    const jobs: Array<{ from: number; to: number; text: string; instruction: string }> = []
    state.doc.descendants((node, pos) => {
      if (!node.isText) return
      const mark = node.marks.find((m) => m.type.name === 'instrField')
      if (!mark) return
      const instruction = String(mark.attrs.instr)
      const next = fieldValue(instruction)
      if (next && next !== node.text) {
        jobs.push({ from: pos, to: pos + node.nodeSize, text: next, instruction })
      }
    })
    if (jobs.length === 0) {
    ...
    const result = updateDocxFields(editor, {
      updates: jobs.map((job) => ({
        range: { from: job.from, to: job.to },
    ...
    setStatus(t('appFieldsUpdated', { n: result.changed }))
  }, [editor, fieldValue])
  ...
  const openPgNumModal = useCallback(() => {
    ...
    setPgNumModal({
      fmt: isDocxPageNumberFormat(sec?.pageNumberFmt) ? sec.pageNumberFmt : 'decimal',
      start: sec?.pageNumberStart !== undefined ? String(sec.pageNumberStart) : '',
    })
  }, [sections, activeSection])

  /** Apply the page-number format to the cursor's section through native history. */
  const applyPgNumFormat = useCallback(() => {
    if (!pgNumModal || !editor) return
    const start =
      pgNumModal.start.trim() === ''
        ? null
        : Math.min(999999, Math.max(0, parseInt(pgNumModal.start, 10) || 0))
    const idx = sections.length > 0 ? Math.min(activeSection, sections.length - 1) : -1
    if (idx >= 0)
      setSectionPageNumbering(
        editor,
        { sectionIndex: idx, format: pgNumModal.fmt, start },
        sections,
      )
    setPgNumModal(null)
    setStatus(t('appPgNumFormatSet'))
  }, [pgNumModal, editor, sections, activeSection])
  ...
  reviewCtxRef.current = {
    ...
    setEndnotes,
    setNotesDirty,
    managedNotes: managedNotesRef.current,
    comments,
    setComments,
    setCommentsDirty,
    setCommentComposing,
  ...
  const singleHfPx = useMemo(() => {
    ...
    return {
      headerPx: estimateHfHeight(
        headerFooterValueAt(0, 'header', 'default'),
        contentW,
        doc?.parsed.headerImages,
      ),
      footerPx: estimateHfHeight(
        headerFooterValueAt(0, 'footer', 'default'),
        contentW,
        doc?.parsed.footerImages,
      ),
    }
  }, [section, headerFooterValueAt, doc])
    ...
    (secs: SectionInfo[]): SectionHfHeights[] => {
      ...
      return secs.map((s, i) => {
        ...
        const pick = (kind: 'header' | 'footer'): HeaderFooter | null =>
          headerFooterValueAt(i, kind, 'default')
  ...
  const mcpRecoveryState: readonly unknown[] = [
    editor?.state.doc,
    sectionDirty,
    section,
    sectionsDirty,
  ...
  useLayoutEffect(() => {
    const previous = mcpRecoveryStateRef.current
    const changed =
      previous === null ||
      previous.length !== mcpRecoveryState.length ||
      mcpRecoveryState.some((value, index) => !Object.is(value, previous[index]))
    if (hasUnsavedChanges && changed) mcpRecoveryVersionRef.current += 1
    mcpRecoveryStateRef.current = mcpRecoveryState
  })
  ...
  useEffect(() => {
    ...
    window.addEventListener('docs-edit-inline-math', handler)
    return () => window.removeEventListener('docs-edit-inline-math', handler)
  }, [editor, doc])
  ...
  // Stable renderer hook for browser tests and local host adapters.
  useEffect(() => {
    window.__genofficeLite = {
      editor,
  ...
  // In an MCP App iframe, route agent mutations into this exact editor instance.
  useEffect(() => {
    if (!editor) return
    ...
    return attachMcpLiveSession({
      execute: async (command) => {
        const registered = await executeDocxOperation(editor, command, {
          allocateListNumId,
          continueList,
          createListDef: createCustomListDef,
          restartList,
          hasCharacterStyle: (styleId) =>
            fileCtxRef.current.doc?.parsed.styles.get(styleId)?.type === 'character',
          hasTableStyle: (styleId) =>
            fileCtxRef.current.doc?.parsed.styles.get(styleId)?.type === 'table',
          resolveSectionBreakSource: (afterBlockIndex) =>
            resolveSectionBreakSource(
              editor,
              afterBlockIndex,
              fileCtxRef.current.sections,
              fileCtxRef.current.section,
            ),
          getSectionLayoutState: () => fileCtxRef.current.sections,
          getDifferentOddEvenPages: () => fileCtxRef.current.evenOddHf,
          getSourceBlocks: () => fileCtxRef.current.doc?.parsed.blocks ?? [],
          getNotes: (kind) =>
            kind === 'footnote' ? fileCtxRef.current.footnotes : fileCtxRef.current.endnotes,
          commitInsertedNote: (kind, id) => {
            managedNotesRef.current.set(`${kind}:${id}`, {
              original: null,
              deleteWhenUnreferenced: true,
            })
            const state = fileCtxRef.current
            const next = collectDocxNotes(
              editor,
              state.footnotes,
              state.endnotes,
            ...
            noteStateRef.current = next
            state.setFootnotes(next.footnotes)
            state.setEndnotes(next.endnotes)
            state.setNotesDirty(true)
          },
          commitUpdatedNote: (kind, id, original) => {
            const key: ManagedDocxNoteKey = `${kind}:${id}`
            if (!managedNotesRef.current.has(key)) {
              managedNotesRef.current.set(key, {
                original,
                deleteWhenUnreferenced: false,
              })
            }
            const state = fileCtxRef.current
            const next = collectDocxNotes(
          ...
          commitDeletedNote: (kind, id, original) => {
            const key: ManagedDocxNoteKey = `${kind}:${id}`
            const baseline = managedNotesRef.current.get(key)?.original ?? original
            managedNotesRef.current.set(key, {
              original: baseline,
              deleteWhenUnreferenced: true,
            })
          ...
          getSources: () => fileCtxRef.current.sources,
          commitSources: (nextSources) => {
            const value = [...nextSources]
            sourceStateRef.current = value
            fileCtxRef.current.setSources(value)
            fileCtxRef.current.setSourcesDirty(true)
          },
          getComments: () => fileCtxRef.current.comments,
          commitComments: (nextComments) => {
            const value = [...nextComments]
            commentStateRef.current = value
            fileCtxRef.current.setComments(value)
            fileCtxRef.current.setCommentsDirty(true)
          },
          commitRevisionTracking: (enabled) => {
            setTrackChanges(enabled)
          },
          getProtection: () => fileCtxRef.current.protection,
          commitProtection: (nextProtection) => {
            const state = fileCtxRef.current
            state.setProtection(nextProtection)
            state.setProtectionDirty(true)
            state.dirtyRef.current = true
          },
          getInkAnnotations: () => fileCtxRef.current.inkAnnotations,
          commitInkAnnotations: (nextAnnotations) => {
            const state = fileCtxRef.current
            state.setInkAnnotations([...nextAnnotations])
            state.setInksDirty(true)
            state.dirtyRef.current = true
          },
          commitCompareResult: (result) => {
            fileCtxRef.current.setCompareResult({
          ...
          commitDocumentDesign: (nextDesign, fields) => {
            commitDocumentDesignState(fileCtxRef.current, nextDesign, fields)
          },
          getHeaderFooterValue: ({ sectionIndex, kind, variant }) => {
            const current = collectSectionSettingsOverrides(editor)
              .find((override) => override.sectionIndex === sectionIndex)
              ?.headerFooterEdits?.find(
                (edit) => edit.kind === kind && edit.variant === variant,
              )?.value
            if (current) return current
            const state = fileCtxRef.current
            const rId = effectiveHfRefs(state.sections)[sectionIndex]?.[kind]?.[variant]
            return rId ? hfFromPart(state.doc?.parsed.hfParts?.[rId]) : null
          },
          loadStaged: (input) => loadStagedDocx(input, loadFile),
          save: async () => {
            if (!(await save(false))) throw new Error('The document was not saved.')
            return { fileName: fileCtxRef.current.doc?.fileName ?? 'document.docx' }
          },
        })
        if (registered.handled) {
          ...
          const output = registered.output ? { output: { ...registered.output } } : {}
          if (registered.checkpointRecovery === false) return { ok: true, ...output }
          const snapshot = await recovery()
          return { ok: true, ...output, ...(snapshot ? { recovery: snapshot } : {}) }
        }
        return {
          ok: false,
      ...
      snapshot: (revision) => ({
        revision,
        fileName: fileCtxRef.current.doc?.fileName ?? null,
        dirty: fileCtxRef.current.dirtyRef.current,
        selection: { ...buildDocxSelectionContext(editor) },
      }),
    })
  }, [editor, loadFile, save, allocateListNumId, continueList, createCustomListDef, restartList])
  ...
  const ribbonActions = useStableCallbacks({
    ...
    onSection: (next: SectionSettings) => {
      if (!editor) return
      const result = setSectionLayoutOverride(editor, activeSection, next, sections)
      if (!result.ok) return
      setSections((prev) =>
    ...
    onPageColor: (next: string | null) => {
      const result = applyDocxDocumentDesign(
        { pageColor, watermark, themeFonts, themeColors },
        { fields: ['pageColor'], pageColor: next },
      )
      if (result.ok && result.changed) {
        commitDocumentDesignState(fileCtxRef.current, result.state, result.changedFields)
      }
    },
    onWatermark: (next: string | null) => {
      const result = applyDocxDocumentDesign(
        { pageColor, watermark, themeFonts, themeColors },
        { fields: ['watermark'], watermark: next },
      )
      if (result.ok && result.changed) {
        commitDocumentDesignState(fileCtxRef.current, result.state, result.changedFields)
      }
    ...
    onThemeFonts: (fonts: ThemeFonts) => {
      const result = applyDocxDocumentDesign(
        { pageColor, watermark, themeFonts, themeColors },
        { fields: ['themeFonts'], themeFonts: fonts },
      )
      if (result.ok && result.changed) {
        commitDocumentDesignState(fileCtxRef.current, result.state, result.changedFields)
      }
    ...
    onThemeColors: (colors: ThemeColors) => {
      const result = applyDocxDocumentDesign(
        { pageColor, watermark, themeFonts, themeColors },
        { fields: ['themeColors'], themeColors: colors },
      )
      if (result.ok && result.changed) {
        commitDocumentDesignState(fileCtxRef.current, result.state, result.changedFields)
      }
    ...
    onAddSource: (source: SourceInfo) => {
      if (!editor) return
      const result = upsertDocxSource(editor, sources, { source })
      if (!result.ok) {
        setStatus(result.message)
        return
      }
    ...
    onHeader: (next: HeaderFooter) => commitHfVariant('header', 'default', next),
    onPageNumFormat: openPgNumModal,
    onInsertField: insertField,
    onFooter: (next: HeaderFooter) => commitHfVariant('footer', 'default', next),
    onPageNumber: (kind: 'header' | 'footer', alignment: 'left' | 'center' | 'right' | null) => {
      if (!editor || sections.length === 0) return
      setHeaderFooterPageNumber(
        editor,
        {
          sectionIndex: hfSectionClamped,
          kind,
          variant: 'default',
        ...
        sections,
        headerFooterValueAt(hfSectionClamped, kind, 'default'),
      )
    },
    onTitlePg: toggleTitlePg,
    onEvenOddHf: (on: boolean) => {
      if (!editor) return
      const result = setDocumentDifferentOddEvenPages(editor, { enabled: on }, sections, evenOddHf)
      if (!result.ok) return
      setEvenOddHf(on)
    ...
    onTrackChanges: (enabled: boolean) => {
      if (!editor) return
      const result = setDocxRevisionTracking(editor, { enabled })
      if (result.ok) setTrackChanges(result.enabled)
    },
...
.editor-scroll .doc-page.measuring-columns { column-count: auto; width: ${colFlow.colWidthPx + twipsToPx(canvasSection?.marginLeft ?? section?.marginLeft ?? 0) + twipsToPx(canvasSection?.marginRight ?? section?.marginRight ?? 0)}px; }`}</style>
      ...
      <Ribbon
        ...
        header={headerFooterValueAt(hfSectionClamped, 'header', 'default')}
        footer={headerFooterValueAt(hfSectionClamped, 'footer', 'default')}
        titlePg={titlePg}
        evenOddHf={evenOddHf}
        showMarks={showMarks}
        showRuler={showRuler}
          ...
          <div className={`workspace ${darkCanvas ? 'workspace-dark' : ''}`}>
            ...
            <div className="editor-area">
              {editorActive ? (
                ...
                <main className="editor-workspace-suspended" aria-hidden="true" />
              )}
            </div>
            {readMode && (
          ...
          <div className="modal">
            ...
            <label className="pgnum-row">
              ...
              <select
                ...
                onChange={(e) =>
                  isDocxPageNumberFormat(e.target.value) &&
                  setPgNumModal({ ...pgNumModal, fmt: e.target.value })
                }
              >
