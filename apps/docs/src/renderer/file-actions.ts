...
import { materializeSectionBreakStarts } from './editor/section-actions'
import { applySectionSettingsOverrides } from './editor/section-layout'
import {
...
import { isBlankDocument } from './editor/is-blank-document'
...
export interface FileActionContext {
  ...
  headerFooterOverrides: Array<{ value: HeaderFooter }>
  titlePg: boolean
...
export async function loadFile(
  ...
  try {
    ...
    const declaredFonts = collectDocFonts(parsed)
    if (window.parent !== window) {
      try {
        await loadBundledFallbackFonts(
          declaredFonts,
          collectDocText(parsed),
          readLiveEditorBundledFontAsset,
        )
      } catch {
        // The editor remains usable with system substitution. The existing
        // missing-font status below makes this degradation visible.
      }
    }
    const missing = checkMissingFonts(declaredFonts)
...
export async function buildDocBytes(ctx: FileActionContext): Promise<Uint8Array | null> {
  ...
  const effectiveSections = applySectionSettingsOverrides(
    ctx.sections,
    plan.sectionSettingsOverrides,
  )
  const overriddenSectionIndexes = plan.sectionSettingsOverrides.map(
    (override) => override.sectionIndex,
  )
  const sectionOverrideByIndex = new Map(
    plan.sectionSettingsOverrides.map((override) => [override.sectionIndex, override]),
  )
  const documentHeaderFooterOverride = plan.sectionSettingsOverrides.find(
    (override) => override.evenOddHeaders !== undefined,
  )
  const finalSectionOverride = plan.sectionSettingsOverrides.find(
    (override) => override.sectionIndex === effectiveSections.length - 1,
  )
  ...
  const dirtySectionIdxs = [...new Set([...ctx.sectionsDirty, ...overriddenSectionIndexes])]
  if (dirtySectionIdxs.length > 0) {
    const rewrites = new Map<number, string>()
    for (const si of dirtySectionIdxs) {
      const sec = effectiveSections[si]
      if (!sec || si === effectiveSections.length - 1) continue
      ...
      const sectionOverride = sectionOverrideByIndex.get(si)
      if (sectionOverride?.titlePg !== undefined) {
        sectPr = applySectionTitlePage(sectPr, sectionOverride.titlePg)
      }
      // Touch w:pgNumType only when the explicit page-number final state was edited.
      if (sectionOverride?.pageNumbering) {
        sectPr = applyPageNumType(
          sectPr,
          sectionOverride.pageNumbering.format === 'decimal'
            ? undefined
            : sectionOverride.pageNumbering.format,
          sectionOverride.pageNumbering.start ?? undefined,
        )
      }
  ...
  const materializedSectionBreaks = materializeSectionBreakStarts(
    saveBlocks,
    plan.sectionBreakStarts,
    doc.parsed.blocks,
  )
  saveBlocks = materializedSectionBreaks.saveBlocks
  // Header/footer content is Undo-owned on section anchors. Non-final sections use
  // indexed part writes; final-section variants use the retained save options.
  const headerFooterEdits = plan.sectionSettingsOverrides.flatMap((override) =>
    (override.headerFooterEdits ?? []).map((edit) => ({
      sectionIndex: override.sectionIndex,
      ...edit,
    })),
  )
  const finalHeaderFooterEdits = headerFooterEdits.filter(
    (edit) => edit.sectionIndex === effectiveSections.length - 1,
  )
  const finalHeaderFooter = (kind: 'header' | 'footer', variant: 'default' | 'first' | 'even') =>
    finalHeaderFooterEdits.find((edit) => edit.kind === kind && edit.variant === variant)?.value
  const sectionHf = headerFooterEdits
    .filter((edit) => edit.sectionIndex < effectiveSections.length - 1)
    .map((edit) => ({
      lastBlockIndex: effectiveSections[edit.sectionIndex].lastBlockIndex,
      kind: edit.kind,
  ...
  const bytes = await saveDocx(doc.parsed, saveBlocks, {
    section:
      ...
      (ctx.sectionDirty || overriddenSectionIndexes.includes(effectiveSections.length - 1))
        ? effectiveSections[effectiveSections.length - 1].settings
        : undefined,
    sectionStartType:
      materializedSectionBreaks.trailingStartType ?? ctx.trailingStartType ?? undefined,
    pgNumType: finalSectionOverride?.pageNumbering
      ? {
          fmt:
            finalSectionOverride.pageNumbering.format === 'decimal'
              ? undefined
              : finalSectionOverride.pageNumbering.format,
          start: finalSectionOverride.pageNumbering.start ?? undefined,
        }
      : undefined,
    ...
    header: finalHeaderFooter('header', 'default') ?? undefined,
    footer: finalHeaderFooter('footer', 'default') ?? undefined,
    headerFirst: finalHeaderFooter('header', 'first') ?? undefined,
    footerFirst: finalHeaderFooter('footer', 'first') ?? undefined,
    headerEven: finalHeaderFooter('header', 'even') ?? undefined,
    footerEven: finalHeaderFooter('footer', 'even') ?? undefined,
    titlePg:
      effectiveSections.length > 0 &&
      sectionOverrideByIndex.get(effectiveSections.length - 1)?.titlePg !== undefined
        ? sectionOverrideByIndex.get(effectiveSections.length - 1)!.titlePg
        : ctx.titlePgDirty
          ? ctx.titlePg
          : undefined,
    evenAndOddHeaders:
      documentHeaderFooterOverride?.evenOddHeaders ??
      (ctx.evenOddHfDirty ? ctx.evenOddHf : undefined),
  ...
  if (
    ...
    hasPrintableHeaderFooter({
      edited: [
        ...
        ...ctx.headerFooterOverrides.map((entry) => entry.value),
      ],
