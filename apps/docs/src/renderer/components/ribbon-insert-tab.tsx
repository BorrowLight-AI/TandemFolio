...
import type { HeaderFooter, NewChart } from '@genoffice/docx-engine'
...
import {
  DEFAULT_DOCX_CHART_HEIGHT_PX,
  DEFAULT_DOCX_CHART_WIDTH_PX,
  insertDocxChartAtPosition,
} from '../editor/chart-actions'
import { startShapeDrawMode } from '../editor/shape-draw'
import { setDocxTextLink } from '../editor/link-actions'
import { isValidDocxBookmarkName, setDocxBookmark } from '../editor/bookmark-actions'
import { insertDocxCrossReference } from '../editor/field-actions'
import { insertDocxText } from '../editor/text-actions'
import { activeTopLevelBlockIndex, setDocxDropCap } from '../editor/drop-cap-actions'
import {
  insertDocxWordArtAfterBlock,
  nextDocxWordArtDrawingId,
  type DocxWordArtPresetId,
...
function collectBookmarks(
  editor: Editor,
): Array<{ name: string; blockIndex: number; pos: number; preview: string }> {
  const out: Array<{ name: string; blockIndex: number; pos: number; preview: string }> = []
  editor.state.doc.forEach((node, pos, blockIndex) => {
    const names = node.attrs?.bookmarks as string[] | null
    if (Array.isArray(names)) {
      for (const name of names) {
        out.push({ name, blockIndex, pos, preview: node.textContent.slice(0, 40) })
      }
    }
  })
  return out
...
function bookmarkTargetAtCursor(editor: Editor): { blockIndex: number } | null {
...
export function BookmarkModal({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  ...
  const addBookmark = () => {
    ...
    if (!isValidDocxBookmarkName(trimmed)) {
    ...
    const result = setDocxBookmark(editor, {
  ...
  const removeBookmark = (entry: { name: string; blockIndex: number }) => {
    const result = setDocxBookmark(editor, {
...
export function CrossRefModal({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  ...
  const insertRef = (b: { name: string; preview: string }) => {
    ...
    const result = insertDocxCrossReference(editor, {
      range: { from, to },
      bookmarkName: b.name,
      displayText: b.preview || b.name,
    })
    if (!result.ok) {
      setError(result.message)
      return
...
export function ChartInsertModal({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  ...
  const insert = () => {
    ...
    if (kind === 'pie') spec.series = spec.series.slice(0, 1)
    const { $from } = editor.state.selection
    const position = $from.depth > 0 ? $from.after(1) : editor.state.selection.to
    const inserted = insertDocxChartAtPosition(
      editor,
      {
        ...spec,
        title: spec.title ?? '',
        widthPx: DEFAULT_DOCX_CHART_WIDTH_PX,
        heightPx: DEFAULT_DOCX_CHART_HEIGHT_PX,
        position,
      },
      t('ribbonChart'),
    )
    if (inserted.ok) onClose()
  }
...
export function LinkInsertModal({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  ...
  const insertLink = () => {
    ...
    editor.commands.focus()
    const unchangedSelection = selectionAtOpen.text && text === selectionAtOpen.text.trim()
    const result = setDocxTextLink(editor, {
      range: unchangedSelection
        ? { from: selectionAtOpen.from, to: selectionAtOpen.to }
        : { from: editor.state.selection.from, to: editor.state.selection.to },
      href,
      text: unchangedSelection ? null : text,
    })
    if (!result.ok) return
    onClose()
  }
              ...
              <div data-rb-panel="" className="wordart-palette">
                {WORDART_PRESETS.map((p) => (
                  <button
                    ...
                    onClick={() => {
                      insertDocxWordArtAfterBlock(editor, {
                        afterBlockIndex: activeTopLevelBlockIndex(editor),
                        preset: p.id as DocxWordArtPresetId,
                        text: t('ribbonWordArtDefaultText'),
                        widthEmu: 2_700_000,
                        heightEmu: 720_000,
                        drawingId: nextDocxWordArtDrawingId(editor),
                      })
                      setDropdown(() => null)
                    }}
                  >
              ...
              <div data-rb-panel="" className="dropcap-menu">
                {(
                  ...
                  <button
                    ...
                    onClick={() => {
                      setDropdown(() => null)
                      setDocxDropCap(editor, {
                        blockIndex: activeTopLevelBlockIndex(editor),
                        mode: opt.val ?? 'none',
                        lines: opt.val ? 3 : null,
                      })
                    }}
                  >
          ...
          <div className="rb-split-wrap">
            ...
            {dropdown === 'pagenum' && (
              <div data-rb-panel="" className="layout-menu">
                ...
                <button
                  onClick={() => {
                    onPageNumber('footer', null)
                    onPageNumber('header', null)
                    setDropdown(() => null)
                  }}
                >
                  {t('ribbonPnRemove')}
