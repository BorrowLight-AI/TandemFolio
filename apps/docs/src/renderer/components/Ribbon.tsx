...
import { EditorFileIcon } from '@genoffice/ui'
import { HIGHLIGHT_CSS } from '../editor/extensions'
import { setSelectedDocxObjectStyle, type DocxObjectStyleField } from '../editor/object-actions'
import {
  replaceSelectedDocxImage,
  setSelectedDocxImageCrop,
  setSelectedDocxImageTransform,
...
export function ribbonTabsForHost(platform: string, nativeFileMenu: boolean) {
  return nativeFileMenu && platform.toLowerCase().includes('mac')
    ? MAC_NATIVE_MENU_TABS
    : STANDARD_TABS
}

// TandemFolio browser and MCP App hosts have no Electron/native file menu.
const HAS_NATIVE_FILE_MENU = false
const USE_MAC_NATIVE_FILE_MENU =
  HAS_NATIVE_FILE_MENU && navigator.platform.toLowerCase().includes('mac')
const TABS = ribbonTabsForHost(navigator.platform, HAS_NATIVE_FILE_MENU)
...
function RibbonInner({
  ...
  const setShapeStyle = (patch: { fill?: string | null; borderColor?: string | null }) => {
    if (!canEdit) return
    const style: { fillHex?: string | null; borderHex?: string | null } = {}
    const fields: DocxObjectStyleField[] = []
    if ('fill' in patch) {
      style.fillHex = patch.fill ?? null
      fields.push('fillHex')
    }
    if ('borderColor' in patch) {
      style.borderHex = patch.borderColor ?? null
      fields.push('borderHex')
    }
    setSelectedDocxObjectStyle(editor, { style, fields })
  }
  ...
  const applyPictureBytes = async (dataUrl: string) => {
    ...
    try {
      ...
      replaceSelectedDocxImage(editor, {
        base64: m[2],
  ...
  const rotatePicture = (deltaDeg: number) => {
    ...
    setSelectedDocxImageTransform(editor, next, !!attrs.imageFlipH, !!attrs.imageFlipV)
  }
  ...
  const flipPicture = (axis: 'h' | 'v') => {
    ...
    setSelectedDocxImageTransform(
      editor,
      Number(attrs.imageRotDeg) || 0,
      axis === 'h' ? !attrs.imageFlipH : !!attrs.imageFlipH,
      axis === 'v' ? !attrs.imageFlipV : !!attrs.imageFlipV,
    )
  }
  ...
  const applyCellBorders = (mode: 'all' | 'outer' | 'inner' | 'none') =>
    runTableCommand(
      setSelectedTableCellBorders(
        mode,
        mode === 'none' ? null : { color: borderColor, sizeEighths: borderSz },
      ),
    )
  ...
  const applyRowHeight = (cm: number | null) => {
    ...
    runTableCommand(setSelectedTableRowHeight(twips))
  }
  ...
  const setTextStyle = (patch: Record<string, unknown>) => {
    ...
    const exact: { format: DocxCharacterFormat; fields: DocxCharacterFormatField[] } | null =
      keys.length === 1 && keys[0] === 'color'
        ? {
            format: { color: patch.color === null ? null : `#${String(patch.color)}` },
            fields: ['color'],
          }
        : keys.length === 1 && keys[0] === 'highlight'
          ? {
              format: { highlight: patch.highlight as DocxCharacterFormat['highlight'] },
              fields: ['highlight'],
            }
          : keys.length === 1 && keys[0] === 'sizeHalfPoints'
            ? {
                format: {
                  fontSizePoints:
                    patch.sizeHalfPoints === null ? null : Number(patch.sizeHalfPoints) / 2,
                },
                fields: ['fontSizePoints'],
              }
            : (keys.length === 1 && (keys[0] === 'font' || keys[0] === 'fontAscii')) ||
                (keys.length === 2 && keys.includes('font') && keys.includes('fontAscii'))
              ? {
                  format: {
                    fontFamily:
                      patch.font === null && patch.fontAscii === null
                        ? null
                        : String(patch.font ?? patch.fontAscii),
                  },
                  fields: ['fontFamily'],
                }
              : keys.length === 1 && keys[0] === 'vertAlign'
                ? {
                    format: {
                      verticalAlign:
                        patch.vertAlign === null
                          ? 'baseline'
                          : (patch.vertAlign as 'superscript' | 'subscript'),
                    },
    ...
    if (from !== to && exact) {
      ed.commands.focus()
      setDocxCharacterFormat(ed, { range: { from, to }, ...exact })
      setDropdown(null)
      return
    }
  ...
  const applyStyle = (key: string) => {
    if (key.startsWith('char:')) {
      ...
      if (preset) {
        ...
        if (from !== to) {
          ...
          if (activeStyleKey === key) {
            setDocxCharacterFormat(ed, {
              range: { from, to },
              format: { [preset.mark]: false, color: null },
              fields: [preset.mark, 'color'],
            })
          } else {
            setDocxCharacterFormat(ed, {
              range: { from, to },
              format: {
                bold: preset.mark === 'bold',
                italic: preset.mark === 'italic',
                color: `#${presetAccent}`,
              },
              fields: ['bold', 'italic', 'color'],
            })
          }
          return
        }
      ...
      if (from !== to) {
        ed.commands.focus()
        setDocxCharacterStyle(ed, {
          range: { from, to },
          styleId: activeCharStyleId === styleId ? null : styleId,
        })
        return
      }
  ...
  const toggleVertAlign = (kind: 'superscript' | 'subscript') => {
    ...
    setDocxCharacterFormat(ed, {
      range: { from, to },
      format: { verticalAlign: fs.vertAlign === kind ? 'baseline' : kind },
      fields: ['verticalAlign'],
    })
  }
  ...
  const changeCase = (mode: 'upper' | 'lower' | 'title' | 'sentence') => {
    ...
    if (from === to) return
    ed.commands.focus()
    transformDocxTextCase(ed, { range: { from, to }, mode })
    setDropdown(null)
  }
  ...
  const markBtn = (
    name: Exclude<DocxCharacterFormatField, 'verticalAlign'>,
    active: boolean,
    title: string,
    label: ReactNode,
  ) => (
  ...
  return (
    <div className="ribbon" ref={ribbonRef}>
      <div
        className={`ribbon-tabs ${IN_TAB ? '' : USE_MAC_NATIVE_FILE_MENU ? 'ribbon-tabs-mac' : 'ribbon-tabs-win'}`}
      >
        {!USE_MAC_NATIVE_FILE_MENU && (
          <div className="file-tab-wrap">
            <button
              className={`ribbon-tab ribbon-tab-file ${dropdown === 'file' ? 'open' : ''}`}
              aria-label={t('ribbonTabFile')}
              title={t('ribbonTabFile')}
              ...
              <div className="table-tool-row">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setSelectedDocxImageWrap(
                      editor,
                      (e.target.value || null) as RetainedDocxImageWrap,
                    )
                  }}
                >
            ...
            <div className="table-tool-group">
              <div className="table-style-gallery">
                <button
                  ...
                  onClick={() => runTableCommand(setSelectedTableStyle(null))}
                >
                ...
                {[...(styles?.values() ?? [])]
                  ...
                  .map((info) => (
                    <button
                      ...
                      onClick={() => runTableCommand(setSelectedTableStyle(info.styleId))}
                    >
                      <span
              ...
              <div className="table-style-swatches">
                {['FFFFFF', 'D9EAF7', 'FFF2CC', 'E2F0D9', 'FCE4D6', 'E4DFEC'].map((hex) => (
                  <button
                    ...
                    style={{ background: `#${hex}` }}
                    onClick={() =>
                      runTableCommand(setSelectedTableCellFormat({ fill: hex }, ['fill']))
                    }
                  />
                ))}
                <button
                  ...
                  onClick={() =>
                    runTableCommand(setSelectedTableCellFormat({ fill: null }, ['fill']))
                  }
                >
                  {t('ribbonNoShading')}
                </button>
              ...
              <div className="table-tool-row">
                {(
                  ...
                  <button
                    ...
                    onClick={() =>
                      runTableCommand(
                        setSelectedTableCellFormat({ verticalAlignment: v }, ['verticalAlignment']),
                      )
                    }
                  >
                    <Icon />
                ...
                <div className="rb-row">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      ed.commands.focus()
                      clearDocxCharacterFormat(ed, { range: { from, to } })
                    }}
                  >
                    <IconClearFormat />
                  </button>
      ...
      {pictureDialog === 'crop' && imageDataUrl && (
        <CropDialog
          ...
          onApply={(crop) => {
            setPictureDialog(null)
            setSelectedDocxImageCrop(editor, crop.left, crop.top, crop.right, crop.bottom)
          }}
          onCancel={() => setPictureDialog(null)}
        />
      )}
