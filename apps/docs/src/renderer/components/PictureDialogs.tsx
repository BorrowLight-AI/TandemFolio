 ...
 * region; applying returns non-destructive source insets. Reuses the modal styles.
 */
...
interface CropProps {
  dataUrl: string
  /** Apply: source crop insets as 0..1 fractions. */
  onApply: (crop: { left: number; top: number; right: number; bottom: number }) => void
  onCancel: () => void
}
...
export function CropDialog({ dataUrl, onApply, onCancel }: CropProps) {
  ...
  const apply = useCallback(() => {
    ...
    onApply({ left: crop.l, top: crop.t, right: 1 - crop.r, bottom: 1 - crop.b })
  }, [crop, onApply])
