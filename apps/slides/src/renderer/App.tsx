...
import { getLiveEditorActivity, subscribeLiveEditorActivity } from '@tandemfolio/host-bridge'
import type {
  GroupRenderNode,
  RenderFill,
  RenderNode,
...
const THUMBS_W_KEY = 'tandemfolio-slides-thumbs-width'
...
export function App() {
  ...
  const [autoSave, setAutoSave] = useState(
    () => localStorage.getItem('tandemfolio-slides-auto-save') === '1',
  )
  useEffect(() => {
    localStorage.setItem('tandemfolio-slides-auto-save', autoSave ? '1' : '0')
  ...
  const saveAs = useCallback(() => setSaveAsName(path?.split('/').pop() || 'Untitled.pptx'), [path])
  const confirmSaveAs = useCallback(() => {
    if (saveAsName === null) return
    const trimmed = saveAsName.trim()
    if (!trimmed || /[/\\]/.test(trimmed)) return
    const fileName = trimmed.toLowerCase().endsWith('.pptx') ? trimmed : `${trimmed}.pptx`
    setSaveAsName(null)
    void fileActions.saveAs(ctxRef.current, fileName)
  }, [saveAsName])
  ...
  // Text selected in plain DOM focuses body; canvas shape selection stays collapsed
  ...
  // window lands directly in the editor on a fresh blank deck.
  ...
  useEffect(() => {
    if (!path) return
    try {
      const raw = localStorage.getItem(`tandemfolio-slides-guides:${path}`)
      setGuides(
        raw
          ? JSON.parse(raw)
          : [
  ...
  useEffect(() => {
    if (!path) return
    try {
      localStorage.setItem(`tandemfolio-slides-guides:${path}`, JSON.stringify(guides))
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(`tandemfolio-slides-custom-shows:${path}`)
  ...
  return (
    <div className="app">
      ...
      <div className="app-main">
        <div className="app-content">
          <div className="workspace">
            {!editorActive ? (
              <div className="workspace-suspended" aria-hidden="true" />
            ) : !slide ? (
      ...
      {saveAsName !== null && (
        <div className="modal-backdrop" onClick={() => setSaveAsName(null)}>
          <form
            className="modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="pptx-save-as-title"
          ...
          >
            <h2 id="pptx-save-as-title">{t('ribbonFileSaveAs')}</h2>
            <label>
              {t('paneCsdName')}
              <input
                autoFocus
            ...
            <div className="modal-actions">
              <button type="button" onClick={() => setSaveAsName(null)}>
                {t('paneCancel')}
              </button>
              <button
                type="submit"
                className="primary"
                disabled={!saveAsName.trim() || /[/\\]/.test(saveAsName)}
              >
                {t('appRehearseSave')}
              </button>
            </div>
