...
import type { GradientFillSpec, LinkTargetOp, SetEffectsPatch } from '../../shared/ipc'
...
interface Props {
  ...
  onImageFill?: (sourceId: string, mode: 'stretch' | 'tile') => void
  ...
  onChangeShape?: (sourceId: string, preset: string) => void
    ...
    <aside className="format-pane">
      ...
      {!node ? (
        ...
        <div className="fp-body">
          {effTab === 'shape' && shapeSub === 'size' && shape?.presetGeometry && onChangeShape && (
            <>
              <div className="fp-section">{t('ribbonChangeShape')}</div>
              <button
                type="button"
                className="fp-btn"
                onClick={() => setShapeGalleryOpen((open) => !open)}
              >
                {t('ribbonChangeShape')}
              </button>
              {shapeGalleryOpen && (
                <ShapeGalleryContent
                  onPick={(preset) => {
                    ...
                    onChangeShape(node.sourceId, preset)
                  }}
                />
              )}
            </>
                ...
                <>
                  <div className="fp-radios">
                    ...
                    {onImageFill &&
                      radioRow(
                        ...
                        fillKind === 'image',
                        t('paneFillImage'),
                        () => onImageFill(node.sourceId, 'stretch'),
                      )}
                  </div>
                  {fillKind === 'solid' && (
                    <>
                  ...
                  {fillKind === 'image' && onImageFill && (
                    <div className="fp-prow fp-prow-end">
                      <button
                        className="fp-btn"
                        onClick={() => onImageFill(node.sourceId, 'stretch')}
                      >
                        {t('paneFormatImageFill')}
                      </button>
                      <button className="fp-btn" onClick={() => onImageFill(node.sourceId, 'tile')}>
                        {t('ribbonFillTexture')}
                      </button>
                    </div>
                  )}
          ...
          {onOpenLink && (
            <>
              ...
              <button type="button" className="fp-btn" onClick={onOpenLink}>
                {t('paneFormatLinkSet')}
              </button>
            </>
          )}
          {onDelete && (
            <>
              ...
              <button
                ...
                onClick={() => onDelete(node.sourceId)}
              >
                {t('paneFormatDelete')}
              </button>
            </>
