...
describe('applySaveRequest', () => {
  ...
  it('replaces and clears only generated watermark/header-footer stamp annotations', async () => {
    const bytes = await makePdf([[612, 792]])
    const stamped = await apply(
      bytes,
      request({
        replaceGeneratedStamps: true,
        stamps: [{ pageIndex: 0, image: TINY_PNG, rect: [0, 0, 612, 792], opacity: 0.2 }],
      }),
    )
    expect(pageAnnots(await PDFDocument.load(stamped), 0).map(subtypeOf)).toEqual(['Stamp'])

    const cleared = await apply(
      stamped,
      request({ replaceGeneratedStamps: true, stamps: [] }),
    )
    expect(pageAnnots(await PDFDocument.load(cleared), 0)).toEqual([])
  })
