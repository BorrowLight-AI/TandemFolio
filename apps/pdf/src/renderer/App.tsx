...
import { AnnotationMode, GlobalWorkerOptions, TextLayer, getDocument } from 'pdfjs-dist/legacy/build/pdf.mjs'
...
import { PDF_OPEN_EVENT } from './host/browser-pdf-api'
import type { PdfCommunityCommandBridge } from './host/community-command-bridge'
...
export default function App({
  ...
  onDocumentOpened?: (path: string, error?: unknown) => void
}): ReactElement {
  ...
  /** Local format-owned author label for comments. */
  const noteAuthor = 'GenOffice'
  ...
  const [documentHistory, setDocumentHistory] = useState({ canUndo: false, canRedo: false })
  ...
  const passwordPathRef = useRef<string | null>(null)
  ...
  const loadDoc = useCallback(
    async (
      ...
      setRedoStack([])
      try {
        setDocumentHistory(await window.pdfApi.documentHistoryState(path))
      } catch {
        setDocumentHistory({ canUndo: false, canRedo: false })
      }
  ...
  const openPath = useCallback(
    async (path: string) => {
      try {
        if (passwordPathRef.current !== path) {
          passwordPathRef.current = path
        ...
        setStatus('error')
        onDocumentOpened?.(path, err)
      }
    },
    [loadDoc, onDocumentOpened],
  )
  ...
  useEffect(() => {
    const openStagedFile = (event: Event) => {
      const path = (event as CustomEvent<{ path?: unknown }>).detail?.path
      if (typeof path === 'string') void openPath(path)
    }
    window.addEventListener(PDF_OPEN_EVENT, openStagedFile)
    return () => window.removeEventListener(PDF_OPEN_EVENT, openStagedFile)
  }, [openPath])
  ...
  /** Latest pushUndo for async callbacks that resume after validation and may otherwise
  ...
  const undo = async () => {
    ...
    if (!top) {
      if (!documentHistory.canUndo) return
      const result = await window.pdfApi.undoDocumentMutation(filePath)
      if (!result.ok) {
        opFailed(result.error)
        return
      }
      setDocumentHistory(await window.pdfApi.documentHistoryState(filePath))
      return
    }
    recoveryVersionRef.current += 1
  ...
  const redo = async () => {
    ...
    if (!top) {
      if (!documentHistory.canRedo) return
      const result = await window.pdfApi.redoDocumentMutation(filePath)
      if (!result.ok) {
        opFailed(result.error)
        return
      }
  ...
  /** Text index cached per document; invalidated after each save reload. */
  ...
  const editsPayload = (
    ...
    replaceGeneratedStamps: stampCfg !== null,
  ...
  /** Bumped whenever the cache is dropped; a late host response started against a
  ...
  useEffect(() => {
    ...
    const assertPage = (pageIndex: number) => {
      if (!Number.isInteger(pageIndex) || pageIndex < 0 || pageIndex >= sizes.length) {
        throw new Error(`Unknown PDF page: ${pageIndex}`)
      }
      if (deleted.has(pageIndex)) throw new Error(`PDF page ${pageIndex} is pending deletion.`)
    }
    const hasPending = (
      ...
      kind === 'markup'
        ? markups.some((entry) => entry.id === id)
        : kind === 'drawing'
          ? drawings.some((entry) => entry.id === id)
          : kind === 'textEdit'
            ? textEdits.some((entry) => entry.id === id)
            : kind === 'textInsert'
              ? textInserts.some((entry) => entry.id === id)
              : imageEdits.some((entry) => entry.id === id)
    ...
    return commandBridge.register({
      ...
      recoverySnapshot: async (force) => {
        ...
        return {
          ...
          data: await window.pdfApi.createRecovery({ path: filePath, ...editsPayload(edits) }),
        }
      },
      addDrawing: async (drawing) => {
        assertPage(drawing.pageIndex)
        const id = newId()
        pushUndo()
        setDrawings((previous) => [...previous, { id, input: drawing }])
        await afterRender()
        return id
      },
      addImageEdit: async (edit, staticFill) => {
        assertPage(edit.pageIndex)
        const id = newId()
        pushUndo()
        setImageEdits((previous) => [...previous, { id, input: edit, staticFill }])
        await afterRender()
        return id
      },
      addMarkup: async (markup) => {
        assertPage(markup.pageIndex)
        const id = newId()
        pushUndo()
        setMarkups((previous) => [...previous, { id, ...markup }])
        await afterRender()
        return id
      },
      addTextInsert: async (text) => {
        assertPage(text.pageIndex)
        const id = newId()
        pushUndo()
        setTextInserts((previous) => [...previous, { id, input: text }])
        await afterRender()
        return id
      },
      deletePage: async (pageIndex) => {
        assertPage(pageIndex)
        if (pageCount <= 1) throw new Error('A PDF must retain at least one page.')
        pushUndo()
        setDeleted((previous) => new Set(previous).add(pageIndex))
        setMarkups((previous) => previous.filter((entry) => entry.pageIndex !== pageIndex))
        setDrawings((previous) => previous.filter((entry) => entry.input.pageIndex !== pageIndex))
        await afterRender()
      },
      deletePending: async (kind, id) => {
        if (!hasPending(kind, id)) throw new Error(`Unknown pending PDF ${kind}: ${id}`)
        pushUndo()
        if (kind === 'markup') setMarkups((previous) => previous.filter((entry) => entry.id !== id))
        else if (kind === 'drawing')
          setDrawings((previous) => previous.filter((entry) => entry.id !== id))
        else if (kind === 'textEdit')
          setTextEdits((previous) => previous.filter((entry) => entry.id !== id))
        else if (kind === 'textInsert')
          setTextInserts((previous) => previous.filter((entry) => entry.id !== id))
        else setImageEdits((previous) => previous.filter((entry) => entry.id !== id))
        setSelected(null)
        await afterRender()
      },
      deleteSavedAnnotation: async (deletion) => {
        assertPage(deletion.pageIndex)
        const annot =
          deletion.subtype === 'note'
            ? (savedNotes.get(deletion.pageIndex) ?? []).find(
                (entry) => entry.objNum === deletion.objNum,
              )
            : (savedMarkups.get(deletion.pageIndex) ?? []).find(
                (entry) => entry.objNum === deletion.objNum,
              )
        if (!annot) throw new Error('The saved PDF annotation is no longer available.')
        pushUndo()
        setAnnotDeletes((previous) => [
          ...previous,
          { id: newId(), annot },
        ])
        await afterRender()
      },
      updateSavedNote: async (edit) => {
        assertPage(edit.pageIndex)
        const annot = (savedNotes.get(edit.pageIndex) ?? []).find(
          (entry) => entry.objNum === edit.objNum,
        )
        if (!annot) throw new Error('The saved PDF comment is no longer available.')
        pushUndo()
        setNoteEdits((previous) => [
          ...previous.filter(
            (entry) =>
              entry.annot.pageIndex !== edit.pageIndex || entry.annot.objNum !== edit.objNum,
          ),
          {
            id: newId(),
            annot,
      ...
      redo: async () => {
        if (redoStack.length === 0 && !documentHistory.canRedo)
          throw new Error('There is no PDF edit to redo.')
        await redo()
        await afterRender()
      },
      replaceText: async (text) => {
        assertPage(text.pageIndex)
        const id = newId()
        const edit = { id, input: text }
        pushUndo()
        setTextEdits((previous) => [...previous, edit])
        validateTextEdit(edit)
        await afterRender()
        return id
      },
      save: () => save(),
      setFormValue: async (value) => {
        if (formCatalog && !formCatalog.fields.has(value.name)) {
          throw new Error(`Unknown PDF form field: ${value.name}`)
        }
        pushUndo(`form:${value.name}`)
        setFormEdits((previous) => new Map(previous).set(value.name, value))
        await afterRender()
      },
      ...
      setPageOrder: async (pageOrder) => {
        const visible = new Set(visList)
        if (
          pageOrder.length !== visList.length ||
          pageOrder.some((pageIndex) => !visible.has(pageIndex))
        ) {
          throw new Error('PDF page order must contain every non-deleted original page once.')
        }
        pushUndo()
        const deletedPages = sizes.map((_, index) => index).filter((index) => !visible.has(index))
        setOrder([...pageOrder, ...deletedPages])
        await afterRender()
      },
      setPageRotation: async (pageIndex, rotation) => {
        assertPage(pageIndex)
        const currentDelta = rotations.get(pageIndex) ?? 0
        const nextDelta = (rotation - (baseRots[pageIndex] ?? 0) + 360) % 360
        if (currentDelta === nextDelta) throw new Error('The PDF page already has that rotation.')
        pushUndo()
        setRotations((previous) => {
          const next = new Map(previous)
          if (nextDelta === 0) next.delete(pageIndex)
          else next.set(pageIndex, nextDelta)
          return next
        })
        const quarterDifference = ((nextDelta - currentDelta + 360) % 360) / 90
        if (quarterDifference % 2 === 1) {
          setDrawings((previous) =>
            previous.map((entry) => {
              if (entry.input.kind !== 'image' || entry.input.pageIndex !== pageIndex) return entry
              const [x1, y1, x2, y2] = entry.input.rect
              const cx = (x1 + x2) / 2
              const cy = (y1 + y2) / 2
              const hw = (x2 - x1) / 2
              const hh = (y2 - y1) / 2
              return {
                ...entry,
                input: { ...entry.input, rect: [cx - hh, cy - hw, cx + hh, cy + hw] },
              }
            }),
          )
        }
      ...
      setStamps: async (watermark, headerFooter) => {
        pushUndo()
        setStampCfg({ wm: watermark, hf: headerFooter })
        setSelected(null)
        await afterRender()
      },
      setStaticForm: async (fill, image, oldRect) => {
        assertPage(fill.pageIndex)
        pushUndo()
        setImageEdits((previous) => {
          const existing = previous.find((entry) => entry.id === fill.id)
          const input: ImageEditInput = oldRect
            ? {
                kind: 'replaceImage',
                pageIndex: fill.pageIndex,
                oldRect,
                rect: fill.rect,
                image,
                layer: 'aboveText',
          ...
          return existing
            ? previous.map((entry) =>
                entry.id === fill.id ? { ...entry, input, staticFill: fill } : entry,
              )
            : [...previous, { id: fill.id, input, staticFill: fill }]
        })
        await afterRender()
      },
      undo: async () => {
        if (undoStack.length === 0 && !documentHistory.canUndo)
          throw new Error('There is no PDF edit to undo.')
        await undo()
        await afterRender()
      },
      updateDrawing: async (id, drawing) => {
        assertPage(drawing.pageIndex)
        if (!drawings.some((entry) => entry.id === id)) {
          throw new Error(`Unknown pending PDF drawing: ${id}`)
        }
        pushUndo()
        setDrawings((previous) =>
          previous.map((entry) => (entry.id === id ? { ...entry, input: drawing } : entry)),
        )
        await afterRender()
      },
      updateTextInsert: async (id, text) => {
        assertPage(text.pageIndex)
        if (!textInserts.some((entry) => entry.id === id)) {
          throw new Error(`Unknown pending inserted PDF text: ${id}`)
        }
        pushUndo()
        setTextInserts((previous) =>
          previous.map((entry) => (entry.id === id ? { ...entry, input: text } : entry)),
        )
        await afterRender()
      },
    })
  ...
  >(new Map())
  /** Last requested render key per page; skips redundant host round-trips. */
      ...
      <div className="app">
        ...
        <PasswordDialog
          ...
          onCancel={() => {
            ...
            setPwWrong(false)
            passwordRef.current = undefined
            passwordPathRef.current = null
            setStatus('empty')
            onDocumentOpened?.(filePath, new Error('Password entry cancelled.'))
          }}
        />
      </div>
    )
      ...
      <div className="ribbon">
        <div className="ribbon-tabs">
          ...
          <button
            ...
            disabled={undoStack.length === 0 && !documentHistory.canUndo}
            onClick={() => void undo()}
          >
            <IconUndo />
          </button>
          <button
            ...
            disabled={redoStack.length === 0 && !documentHistory.canRedo}
            onClick={() => void redo()}
          >
            <IconRedo />
          </button>
