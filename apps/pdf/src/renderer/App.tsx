...
function useVisibleSet(
  ...
  useEffect(() => {
    ...
    if (!enabled) {
      setVisible((previous) => (previous.size === 0 ? previous : new Set()))
      return
    }
...
export default function App({
  ...
  useEffect(() => {
    ...
    return commandBridge.register({
        ...
        setAnnotDeletes((previous) => [
          ...previous,
          {
            ...
            annot: {
              ...
              objNum: deletion.objNum,
              type: deletion.subtype,
              rect: deletion.rect,
              quads: [],
            },
          },
      ...
      redo: async () => {
        if (redoStack.length === 0) throw new Error('There is no PDF edit to redo.')
        redo()
        await afterRender()
      },
      ...
      undo: async () => {
        if (undoStack.length === 0) throw new Error('There is no PDF edit to undo.')
        undo()
        await afterRender()
      },
