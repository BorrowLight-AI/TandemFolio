...
import {
  exportDocxBytes,
  loadMarkdownImageForDocx,
  mapDocToSaveBlocks,
} from '../src/renderer/export/docxExport'
...
describe('docx export', () => {
  it('loads a bounded PNG data URL with its intrinsic dimensions', async () => {
    const image = await loadMarkdownImageForDocx(
      'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M/wHwAF/gL+Xw4bWQAAAABJRU5ErkJggg==',
    )
    expect(image).toMatchObject({ mime: 'image/png', widthPx: 1, heightPx: 1 })
    expect(image?.base64).toMatch(/^iVBOR/)
  })

  it('embeds a data-URL image into exported DOCX media', async () => {
    const editor = createEditor(
      '![pixel](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M/wHwAF/gL+Xw4bWQAAAABJRU5ErkJggg==)',
    )
    const parsed = await parseDocx(
      await exportDocxBytes(editor.getJSON(), loadMarkdownImageForDocx),
    )
    expect(parsed.blocks.some((block) => block.type === 'image')).toBe(true)
  })
