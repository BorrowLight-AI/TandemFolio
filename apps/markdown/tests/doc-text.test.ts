...
} from '../src/renderer/editor/document-preference-actions'

describe('Markdown document preferences', () => {
  it('persists and reads an explicit autosave final state', () => {
    ...
    expect(setMarkdownAutoSavePreference(true, storage)).toBe(true)
    expect(readMarkdownAutoSavePreference(storage)).toBe(true)
    expect(setMarkdownAutoSavePreference(false, storage)).toBe(false)
    expect(readMarkdownAutoSavePreference(storage)).toBe(false)
  })
})
