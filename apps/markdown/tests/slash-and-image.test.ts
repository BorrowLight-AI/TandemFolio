...
import {
  hydrateMarkdownLocalImages,
  resolveImageSrc,
  unresolveImageSrc,
} from '../src/renderer/editor/localImage'
...
describe('Markdown local image hydration', () => {
  it('uses bridged bytes for display while preserving the authored path on serialization', async () => {
    ...
    editor
      ...
      .setContent('![pixel](assets/pixel.png)', { contentType: 'markdown' })
      .run()
    editors.push(editor)
    const bytes = Uint8Array.from([0x89, 0x50, 0x4e, 0x47])

    await hydrateMarkdownLocalImages(editor, 'root-1', async (rootId, path) => {
      expect(rootId).toBe('root-1')
      expect(path).toBe('assets/pixel.png')
      return { mime: 'image/png', data: bytes.buffer }
    })

    const image = editor.state.doc.child(0)
    expect(image.attrs.src).toBe('data:image/png;base64,iVBORw==')
    expect(image.attrs.authoredSrc).toBe('assets/pixel.png')
    expect(editor.getMarkdown()).toContain('![pixel](assets/pixel.png)')
