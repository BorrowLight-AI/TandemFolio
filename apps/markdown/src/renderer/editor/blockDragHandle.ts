  ...
  return new Plugin({
    ...
    props: {
      handleDrop(view, event) {
        ...
        const targetPosition = topLevelPosAt(view, { left: event.clientX, top: event.clientY })
    ...
    view(view) {
      ...
      const onDragStart = (event: DragEvent) => {
        ...
        draggedBlockIndex = view.state.doc.resolve(selection.from).index(0)
      ...
      const onPlusClick = () => {
        ...
        const selection = selectHovered()
        if (!selection) return
        const blockIndex = view.state.doc.resolve(selection.from).index(0)
        updateMarkdownBlock(editor, {
          blockIndex,
          action: 'add_below',
