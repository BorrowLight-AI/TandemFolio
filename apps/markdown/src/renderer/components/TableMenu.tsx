...
export function TableMenu({ editor, scrollRef }: Props) {
  ...
  const run = (action: MarkdownTableUpdateAction, headerRow: boolean | null = null) =>
    updateMarkdownTable(editor, {
  ...
  return (
    <div
      ...
      onMouseDown={(e) => e.preventDefault()}
    >
      <Btn title={t('tableRowAbove')} onClick={() => run('add_row_before')}>
        <IconRowInsertAbove size={ICON} />
      </Btn>
      <Btn title={t('tableRowBelow')} onClick={() => run('add_row_after')}>
        <IconRowInsertBelow size={ICON} />
      </Btn>
      <Btn title={t('tableDeleteRow')} danger onClick={() => run('delete_row')}>
        <IconRowDelete size={ICON} />
      </Btn>
      <span className="tm-sep" />
      <Btn title={t('tableColLeft')} onClick={() => run('add_column_before')}>
        <IconColInsertLeft size={ICON} />
      </Btn>
      <Btn title={t('tableColRight')} onClick={() => run('add_column_after')}>
        <IconColInsertRight size={ICON} />
      </Btn>
      <Btn title={t('tableDeleteCol')} danger onClick={() => run('delete_column')}>
      ...
      <Btn
        ...
        onClick={() =>
          run(
            'set_header_row',
            !(markdownTableHasHeaderRow(editor, editor.state.selection.from) ?? false),
          )
        }
      >
        <IconHeaderRow size={ICON} />
      </Btn>
      <Btn title={t('tableDeleteTable')} danger onClick={() => run('delete_table')}>
