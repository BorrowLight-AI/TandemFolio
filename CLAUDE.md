...
Additional guidance for AI agents and human contributors. [AGENTS.md](AGENTS.md) is the primary repository policy; use [project facts and attribution](docs/project-facts.md) for source and license requirements.
...
`useI18n()`'s `t` is not referentially stable; never put it in a hook dependency array. Store the key and translate at render time.
...
The repository uses the default five canonical triage labels. See `docs/agents/triage-labels.md`.
