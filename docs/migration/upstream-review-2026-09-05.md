...
- 按用户确认，完整移植候选中适用于浏览器挂载权威边界的 DOCX 原生编辑、排版、渲染、导入导出与保真能力。采用选择性来源移植，不创建 merge parent，也不把五格式整体源码固定点提升到候选提交。
- 明确排除 AI 面板/工具/提示词、账号、遥测、Electron main/preload、IPC、桌面系统打印、整文件密码加密和企业版来源。现有 DOCX OOXML 文档保护及其 typed MCP 路由保留。
- DOCX 新增 `docx.image.set_z_order`；生成目录现为 103 个 DOCX 操作、338 个总操作。它与右键 Arrange 共用挂载编辑器状态、单次原生 Undo、恢复及保存路径。
...
| `ee/` | 禁止检查和导入 |
| 上游 i18n 目录拆分、共享 ColorPicker/font-list 的结构性搬家 | 本地已有无 AI 的等价语言表、完整 Office 色板和字体列表；保留现有结构避免把 AI catalog 带回运行图 |

DOCX 候选范围内没有剩余待移植的适用原生能力。XLSX 的 1904 日期系统、手动分页符、分页预览、精确打印设置和首页/奇偶页页眉页脚布局已移植；后续清单保留全文件查找、筛选一致性、错误扫描、CSV/工作簿合并和 RTL，以及 PPTX 图表/SmartArt/3D/WordArt/嵌入字体与复杂 RTL、PDF 评论/文字编辑保真/阅读位置、Markdown 数学公式与资源生命周期。这些均需要各格式自有 Registry 和浏览器状态适配。
...
| [afc6711](https://github.com/genspark-ai/genoffice/commit/afc6711) | 2026-08-24 | fix(sheets): extend Ctrl+F beyond the loaded window on streamed workbooks (#131) | 后续：全文件查找依赖上游文件流式扫描接口；本地浏览器工作簿适配需独立审查。 |
| [cc8cff4](https://github.com/genspark-ai/genoffice/commit/cc8cff4) | 2026-08-24 | feat(sheets): cross-highlight the active cell's row and column (#132) | 后续小特性：活动行列高亮，需接入本地视图状态与能力契约。 |
...
| [7a814db](https://github.com/genspark-ai/genoffice/commit/7a814db) | 2026-08-25 | fix(sheets): keep filter-hidden rows out of full-sheet find results (#133) | 后续：依赖 #131 的全文件查找实现。 |
...
| [7eb5d59](https://github.com/genspark-ai/genoffice/commit/7eb5d59) | 2026-08-26 | fix(sheets): scan the whole file for error checking on streamed workbooks (#134) | 后续：依赖文件分页扫描，需适配现有 XLSX 状态和错误检查路由。 |
...
| [9f971ed](https://github.com/genspark-ai/genoffice/commit/9f971ed) | 2026-09-03 | Sync snapshot (2026-09-02) (#181) | DOCX 非 AI 原生部分已选择性移植；AI/宿主及其他格式部分按边界排除或后续评估。 |
...
- `npm run typecheck` 与 `npm run operation-manifest:check` 通过；生成 Manifest 为 338 个操作，其中 DOCX 103 个。
- 各根工作区测试通过：289 个测试文件、4,085 项；另有 1 个环境条件文件/断言跳过。首次运行准确暴露新增层级命令造成的固定清单与 Skill 文档漂移，补齐后 MCP server 的 13 个文件、385 项全量通过。
...
- `npm run smoke:mcp`：25 个总工具（10 个公开、15 个 app-only）冒烟通过。
- `npm run measure:assets`：五格式通过。DOCX 实测 3,508,575 raw / 987,885 gzip；为已审计的分页、字体和原生 UI 增量把 raw 回归上限从 3,500,000 调为 3,650,000。`npm run licenses` 与 `npm run upstream:check` 通过。
