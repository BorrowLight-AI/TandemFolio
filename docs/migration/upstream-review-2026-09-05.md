...
DOCX 候选范围内没有剩余待移植的适用原生能力。后续清单只保留其他格式的上游大功能：PPTX 图表/SmartArt/3D/WordArt/嵌入字体与复杂 RTL；XLSX 全文件查找、筛选一致性、错误扫描、CSV/合并/RTL/打印页眉页脚；PDF 评论、文字编辑保真与阅读位置；Markdown 数学公式与资源生命周期。这些均需要各格式自有 Registry 和浏览器状态适配。
