...
DOCX 候选范围内没有剩余待移植的适用原生能力。XLSX 的 1904 日期系统、手动分页符和分页预览已移植；后续清单保留全文件查找、筛选一致性、错误扫描、CSV/工作簿合并、RTL、完整打印页眉页脚，以及 PPTX 图表/SmartArt/3D/WordArt/嵌入字体与复杂 RTL、PDF 评论/文字编辑保真/阅读位置、Markdown 数学公式与资源生命周期。这些均需要各格式自有 Registry 和浏览器状态适配。
