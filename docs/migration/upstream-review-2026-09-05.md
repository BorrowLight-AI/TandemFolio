...
- 按用户确认，完整移植候选中适用于浏览器挂载权威边界的 DOCX 与 XLSX 原生编辑、排版、渲染、导入导出与保真能力。采用选择性来源移植，不创建 merge parent，也不把五格式整体源码固定点提升到候选提交。
- 明确排除 AI 面板/工具/提示词、账号、遥测、Electron main/preload、IPC、桌面系统打印、整文件密码加密和企业版来源。现有 DOCX OOXML 文档保护及其 typed MCP 路由保留。
- DOCX 新增 `docx.image.set_z_order`，XLSX 新增计算、保护、分页符、名称和工作簿合并路由；生成目录现为 103 个 DOCX 操作、123 个 XLSX 操作、347 个总操作。它们与各自原生 UI 共用挂载编辑器状态、Undo、恢复及保存路径。
...
DOCX 与 XLSX 候选范围内没有剩余待移植的适用原生能力。XLSX 已完成全文件查找、筛选一致性、错误扫描、活动行列高亮、CSV 导出、工作簿合并、RTL、OLE、Office 配色、1904 日期系统、手动分页符、分页预览、精确打印设置和首页/奇偶页页眉页脚布局。后续清单只保留 PPTX 图表/SmartArt/3D/WordArt/嵌入字体与复杂 RTL、PDF 评论/文字编辑保真/阅读位置、Markdown 数学公式与资源生命周期。
...
- `npm run typecheck` 与 `npm run operation-manifest:check` 通过；生成 Manifest 为 347 个操作，其中 DOCX 103 个、XLSX 123 个。
