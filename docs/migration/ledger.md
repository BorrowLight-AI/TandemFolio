...
103 DOCX / 117 XLSX / 341 total operations;
...
| `apps/sheets/src/renderer`                                                                 | Selectively retained/adapted   | Candidate-native XLSX migration in progress; release recapture pending | `main.tsx` mounts the community `App.tsx` directly. The 117-operation XLSX registry (115 Agent-visible, two internal) includes typed calculation-mode, recalculation, and Goal Seek routes through mounted Univer state. Candidate-native formula correctness, structural guards, rendering, protection/theme helpers, printing, keyboard clearing, and CSV serialization are being admitted in vertical TDD slices. Source-current release evidence remains fail-closed while this migration changes the renderer. See [`xlsx-capability-inventory.md`](xlsx-capability-inventory.md). |
...
- Operation registry foundation: fixture and real multi-format product manifests pass deterministic
  `--check`. The generated Manifest contains 341 operations: 103 DOCX, 22 Markdown, 117 XLSX, 74
