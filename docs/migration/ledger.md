...
103 DOCX / 119 XLSX / 343 total operations;
...
| `apps/sheets/src/renderer`                                                                 | Selectively retained/adapted   | Candidate-native XLSX migration in progress; release recapture pending | `main.tsx` mounts the community `App.tsx` directly. The 119-operation XLSX registry (117 Agent-visible, two internal) includes typed calculation-mode, recalculation, Goal Seek, native workbook-theme, and workbook-structure-protection routes through mounted Univer/file-journal state. Candidate-native formula correctness, structural guards, rendering, remaining protection metadata, printing, keyboard clearing, and CSV serialization are being admitted in vertical TDD slices. Source-current release evidence remains fail-closed while this migration changes the renderer. See [`xlsx-capability-inventory.md`](xlsx-capability-inventory.md). |
...
- Operation registry foundation: fixture and real multi-format product manifests pass deterministic
  `--check`. The generated Manifest contains 343 operations: 103 DOCX, 22 Markdown, 119 XLSX, 74
