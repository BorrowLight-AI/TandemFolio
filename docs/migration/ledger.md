...
103 DOCX / 118 XLSX / 342 total operations;
...
| `apps/sheets/src/renderer`                                                                 | Selectively retained/adapted   | Candidate-native XLSX migration in progress; release recapture pending | `main.tsx` mounts the community `App.tsx` directly. The 118-operation XLSX registry (116 Agent-visible, two internal) includes typed calculation-mode, recalculation, Goal Seek, and native workbook-theme routes through mounted Univer/file-journal state. Candidate-native formula correctness, structural guards, rendering, protection metadata, printing, keyboard clearing, and CSV serialization are being admitted in vertical TDD slices. Source-current release evidence remains fail-closed while this migration changes the renderer. See [`xlsx-capability-inventory.md`](xlsx-capability-inventory.md). |
...
- Operation registry foundation: fixture and real multi-format product manifests pass deterministic
  `--check`. The generated Manifest contains 342 operations: 103 DOCX, 22 Markdown, 118 XLSX, 74
