...
`docx.image.set_z_order` and the two XLSX calculation routes raise the generated Registry to
103 DOCX / 116 XLSX / 340 total operations;
...
| `apps/sheets/src/renderer`                                                                 | Selectively retained/adapted   | Candidate-native XLSX migration in progress; release recapture pending | `main.tsx` mounts the community `App.tsx` directly. The 116-operation XLSX registry (114 Agent-visible, two internal) includes typed calculation-mode and recalculation routes through mounted Univer state. Candidate-native formula correctness, structural guards, rendering, protection/theme helpers, printing, keyboard clearing, and CSV serialization are being admitted in vertical TDD slices. Source-current release evidence remains fail-closed while this migration changes the renderer. See [`xlsx-capability-inventory.md`](xlsx-capability-inventory.md). |
...
- Operation registry foundation: fixture and real multi-format product manifests pass deterministic
  `--check`. The generated Manifest contains 340 operations: 103 DOCX, 22 Markdown, 116 XLSX, 74
