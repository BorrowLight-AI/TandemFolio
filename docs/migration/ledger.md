...
103 DOCX / 121 XLSX / 345 total operations;
...
| `apps/sheets/src/renderer`                                                                 | Selectively retained/adapted   | Candidate-native XLSX migration in progress; release recapture pending | `main.tsx` mounts the community `App.tsx` directly. The 121-operation XLSX registry (119 Agent-visible, two internal) includes typed calculation-mode, recalculation, Goal Seek, native themes, workbook protection, allow-edit ranges, and manual page breaks through mounted Univer/file-journal state. Candidate-native formula correctness, structural guards, native style hydration, shrink-to-fit rendering, printing, keyboard clearing, and CSV serialization are being admitted in vertical TDD slices. Source-current release evidence remains fail-closed while this migration changes the renderer. See [`xlsx-capability-inventory.md`](xlsx-capability-inventory.md). |
...
- Tests: the root workspaces execute 327 passing test files plus one environment-conditional skip,
  for 4,304 passing assertions and one skipped assertion. Workspace assertions are Operation
  Contract 24, Host Bridge 44, DOCX 1,680, Markdown 127, XLSX 1,606 plus one environment skip,
