...
- Capability status: 119 XLSX registry operations (117 Agent-visible, two internal); the candidate-native migration adds calculation mode, explicit recalculation, Goal Seek, native workbook themes, and workbook structure protection while retaining the audited baseline command surface
...
structural mutations continue through the mounted Univer state and native Undo. Page Layout theme,
