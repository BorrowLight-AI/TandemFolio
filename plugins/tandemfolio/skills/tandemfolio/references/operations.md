...
- `docx.image.set_z_order { imageBlockIndex, zOrder, floatInline }`: set one image's signed 32-bit stacking rank. `floatInline: true` converts an inline image to front-of-text before applying the rank; the renderer records the change as one native Undo unit.
